...
/*!
    \file   nanovdb_editor/compiler/SlangCompiler.cpp
...
{
    return (type >= 0 && type < sizeof(scalarTypeNames) / sizeof(scalarTypeNames[0])) ? scalarTypeNames[type] : "float";
}
...
static ResourceTypeSlangDesc resourceTypeFromSlang[resourceTypeDescCount] = {
    ...
    { PNANOVDB_COMPUTE_DESCRIPTOR_TYPE_CONSTANT_BUFFER, SLANG_TYPE_KIND_CONSTANT_BUFFER, SLANG_RESOURCE_NONE,
      SLANG_RESOURCE_ACCESS_NONE },
    { PNANOVDB_COMPUTE_DESCRIPTOR_TYPE_STRUCTURED_BUFFER, SLANG_TYPE_KIND_RESOURCE, SLANG_STRUCTURED_BUFFER,
      SLANG_RESOURCE_ACCESS_READ },
    { PNANOVDB_COMPUTE_DESCRIPTOR_TYPE_RW_STRUCTURED_BUFFER, SLANG_TYPE_KIND_RESOURCE, SLANG_STRUCTURED_BUFFER,
      SLANG_RESOURCE_ACCESS_READ_WRITE },
    { PNANOVDB_COMPUTE_DESCRIPTOR_TYPE_BUFFER, SLANG_TYPE_KIND_RESOURCE, SLANG_TEXTURE_BUFFER,
      SLANG_RESOURCE_ACCESS_READ },
    ...
    { PNANOVDB_COMPUTE_DESCRIPTOR_TYPE_SAMPLER, SLANG_TYPE_KIND_SAMPLER_STATE, SLANG_RESOURCE_NONE,
      SLANG_RESOURCE_ACCESS_NONE },
    { PNANOVDB_COMPUTE_DESCRIPTOR_TYPE_RW_BUFFER, SLANG_TYPE_KIND_RESOURCE, SLANG_RESOURCE_NONE,
      SLANG_RESOURCE_ACCESS_READ_WRITE },
    { PNANOVDB_COMPUTE_DESCRIPTOR_TYPE_RW_TEXTURE, SLANG_TYPE_KIND_RESOURCE, SLANG_TEXTURE_1D,
      SLANG_RESOURCE_ACCESS_READ_WRITE },
    { PNANOVDB_COMPUTE_DESCRIPTOR_TYPE_RW_TEXTURE, SLANG_TYPE_KIND_RESOURCE, SLANG_TEXTURE_2D,
      SLANG_RESOURCE_ACCESS_READ_WRITE },
    { PNANOVDB_COMPUTE_DESCRIPTOR_TYPE_RW_TEXTURE, SLANG_TYPE_KIND_RESOURCE, SLANG_TEXTURE_3D,
      SLANG_RESOURCE_ACCESS_READ_WRITE },
};

static pnanovdb_compute_descriptor_type_t resourceTypeFromSlangLookup(SlangTypeKind typeKind,
                                                                      SlangResourceShape resourceShape,
                                                                      SlangResourceAccess access)
{
                                ...
                                const pnanovdb_compiler_settings_t* settings)
{
    ...
    if (outFile.is_open())
    {
        ...
        outFile.write(
            static_cast<const char*>(shader_->computeShader.byteCode.get()), shader_->computeShader.byteCodeSize);
#else
        outFile << oss.str();
#endif
        outFile.close();
...
bool SlangCompiler::compile(const pnanovdb_compiler_settings_t* settings,
...
#define ENTRY_CNT 5
    ...
    if (shaderParams && shaderParams->getKind() == slang::TypeReflection::Kind::Struct)
    {
        for (uint32_t i = 0; i < shaderParams->getFieldCount(); i++)
        {
            ...
            shader_->shaderParameters.push_back({ name, { typeName, elementCount } });
        }
    }
    ...
    if (settings->compile_target == PNANOVDB_COMPILE_TARGET_CPU)
    {
        ...
        // const SlangResult sharedLibResult = composedProgram->getEntryPointHostCallable(entryPointIndex, targetIndex,
    ...
    else
    {
        ...
        if (computeShaderBlob)
        {
            ...
            pnanovdb_shader::ByteCodePtr byteCode(
                new char[shader_->computeShader.byteCodeSize], std::default_delete<char[]>());
...
ShaderDataPtr compileShader(SlangCompiler& compiler,
                            const pnanovdb_compute_shader_source_t* source,
                            const pnanovdb_compiler_settings_t* settings)
{
...
bool executeCpu(const SlangCompiler& compiler,
                const pnanovdb_shader::ShaderDesc& shader,
                ...
                void* uniformState)
{
    ...
    func(&varyingInput, uniformParams, uniformState);
