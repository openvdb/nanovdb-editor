...
/*!
    \file   nanovdb_editor/compiler/Compiler.cpp
...
pnanovdb_bool_t compile_file(pnanovdb_compiler_instance_t* instance,
                             const char* filename,
                             ...
                             pnanovdb_bool_t* shader_updated)
{
    ...
    SlangCompiler* compilerPtr = cast(instance);
    if (!compilerPtr)
    {
        std::lock_guard<std::mutex> lock(g_slang_mutex);

        compilerPtr = cast(create_instance());
    }

    pnanovdb_compute_shader_source_t source = {};
    source.source = nullptr;
    source.source_filename = filename;
    source.get_source_include_userdata = (void*)NANOVDB_EDITOR_INCLUDES_FILE;
...
pnanovdb_bool_t execute_cpu(pnanovdb_compiler_instance_t* instance,
                            const char* filename,
