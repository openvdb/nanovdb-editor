...
if "%USE_VCPKG%"=="ON" (
    ...
    set "PY_VCPKG_PREFIX_PATH=%VCPKG_PREFIX_PATH:\=/%"
    set "PY_VCPKG_INSTALLED_DIR=%VCPKG_INSTALLED_DIR:\=/%"
    set "PY_PROJECT_DIR=%PROJECT_DIR:\=/%"
    set "CMAKE_ARGS=-DCMAKE_TOOLCHAIN_FILE=%PY_VCPKG_CMAKE% -DCMAKE_PREFIX_PATH=%PY_VCPKG_PREFIX_PATH% -DVCPKG_INSTALLED_DIR=%PY_VCPKG_INSTALLED_DIR% -DVCPKG_TARGET_TRIPLET=%VCPKG_TRIPLET% -DVCPKG_MANIFEST_DIR=%PY_PROJECT_DIR% -DNANOVDB_EDITOR_USE_VCPKG=ON"
) else (
    set "CMAKE_ARGS="
)
