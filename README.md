...
Shader-backed parameters are shown in the object properties/shader parameter UI. Custom scene params are editor-only, loaded per scene through `set_custom_scene_params(editor, scene, json_token, error_buf, error_buf_size)` where `json_token->str` contains the JSON payload; on failure the function returns `PNANOVDB_FALSE` and writes a human-readable message into `error_buf`. Custom scene params are rendered in the dedicated `Scene Params` window docked above the scene tree. They do not currently change object `map_shader_params` output.
