...
{
    FormatConverterElement formats_pnanovdb[PNANOVDB_COMPUTE_FORMAT_COUNT] = { { PNANOVDB_COMPUTE_FORMAT_UNKNOWN,
                                                                                 VK_FORMAT_UNDEFINED, 0u } };
    FormatConverterElement formats_vulkan[2u * PNANOVDB_COMPUTE_FORMAT_COUNT] = { { PNANOVDB_COMPUTE_FORMAT_UNKNOWN,
                                                                                    VK_FORMAT_UNDEFINED, 0u } };
};

void formatConverter_placeElement(FormatConverter* ptr,
                                  pnanovdb_compute_format_t format_pnanovdb,
                                  VkFormat format_vulkan,
                                  pnanovdb_uint32_t size_in_bytes)
{
    ...
    if (format_vulkan < PNANOVDB_COMPUTE_FORMAT_COUNT)
    {
        ptr->formats_vulkan[format_vulkan] = e;
    }
    else
    {
        ptr->formats_vulkan[PNANOVDB_COMPUTE_FORMAT_COUNT + (format_vulkan % PNANOVDB_COMPUTE_FORMAT_COUNT)] = e;
    }
}
...
FormatConverter* formatConverter_create()
{
    ...
    // video
    formatConverter_placeElement(
        ptr, PNANOVDB_COMPUTE_FORMAT_G8_B8R8_2PLANE_420_UNORM, VK_FORMAT_G8_B8R8_2PLANE_420_UNORM, 1u);
    formatConverter_placeElement(
        ptr, PNANOVDB_COMPUTE_FORMAT_G8_B8_R8_3PLANE_420_UNORM, VK_FORMAT_G8_B8_R8_3PLANE_420_UNORM, 1u);
...
{
    if (format < PNANOVDB_COMPUTE_FORMAT_COUNT)
    {
        return ptr->formats_vulkan[format].format_pnanovdb;
    }
    return ptr->formats_vulkan[PNANOVDB_COMPUTE_FORMAT_COUNT + (format % PNANOVDB_COMPUTE_FORMAT_COUNT)].format_pnanovdb;
}
...
#else
    PNANOVDB_VULKAN_TRY_ENABLE_INSTANCE_EXTENSION(VK_KHR_XCB_SURFACE);
    PNANOVDB_VULKAN_TRY_ENABLE_INSTANCE_EXTENSION(VK_KHR_WAYLAND_SURFACE);
#endif
...
#endif
    if (instanceSupportsVulkanVideo)
    {
        PNANOVDB_VULKAN_TRY_ENABLE_DEVICE_EXTENSION(VK_KHR_VIDEO_QUEUE);
        PNANOVDB_VULKAN_TRY_ENABLE_DEVICE_EXTENSION(VK_KHR_VIDEO_ENCODE_QUEUE);
    }
    PNANOVDB_VULKAN_TRY_ENABLE_DEVICE_EXTENSION(VK_KHR_MAINTENANCE_5);
    PNANOVDB_VULKAN_TRY_ENABLE_DEVICE_EXTENSION(VK_EXT_SHADER_64BIT_INDEXING);
...
#undef PNANOVDB_VULKAN_TRY_ENABLE_DEVICE_EXTENSION
    ...
    // determine what requested extensions are supported
    {
        ...
        ptr->deviceManager->loader.vkEnumerateDeviceExtensionProperties(
            ptr->physicalDevice, nullptr, &numDeviceExtensions, nullptr);

        deviceExtensions.reserve(numDeviceExtensions);
        deviceExtensions.resize(numDeviceExtensions);

        ptr->deviceManager->loader.vkEnumerateDeviceExtensionProperties(
            ptr->physicalDevice, nullptr, &numDeviceExtensions, deviceExtensions.data());
