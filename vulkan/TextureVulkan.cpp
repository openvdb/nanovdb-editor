...
void texture_createImageView(Context* context,
                             Texture* ptr,
                             VkImageView* view_all,
                             VkImageView* view_mipLevel,
                             pnanovdb_compute_format_t format,
                             pnanovdb_compute_texture_aspect_t aspect)
{
    if (aspect == PNANOVDB_COMPUTE_TEXTURE_ASPECT_NONE)
    {
        aspect = PNANOVDB_COMPUTE_TEXTURE_ASPECT_COLOR;
    }
    ...
    viewInfo.format = formatConverter_convertToVulkan(context->deviceQueue->device->formatConverter, format);
    ...
    if (format == PNANOVDB_COMPUTE_FORMAT_G8_B8R8_2PLANE_420_UNORM ||
        format == PNANOVDB_COMPUTE_FORMAT_G8_B8_R8_3PLANE_420_UNORM)
    {
        viewUsageInfo.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_USAGE_CREATE_INFO;
        if (aspect == PNANOVDB_COMPUTE_TEXTURE_ASPECT_COLOR)
        {
...
void texture_pushImageView(Context* context,
                           Texture* ptr,
                           pnanovdb_compute_format_t format,
                           pnanovdb_compute_texture_aspect_t aspect)
{
    ...
    texture_createImageView(context, ptr, &imageViewVk_all, &imageViewVk_mipLevel, format, aspect);

    ptr->aliasFormats.push_back(format);
    ptr->aliasAspects.push_back(aspect);
                                    ...
                                    pnanovdb_compute_texture_aspect_t aliasAspect)
{
    ...
    {
        if (ptr->aliasFormats[idx] == aliasFormat && ptr->aliasAspects[idx] == aliasAspect)
        {
            return ptr->aliasImageViewAlls[idx];
        }
    }
    texture_pushImageView(context, ptr, aliasFormat, aliasAspect);
    return texture_getImageViewAll(context, ptr, aliasFormat, aliasAspect);
}
                                         ...
                                         pnanovdb_compute_texture_aspect_t aliasAspect)
{
    ...
    texture_pushImageView(context, ptr, aliasFormat, aliasAspect);
    return texture_getImageViewMipLevel(context, ptr, aliasFormat, aliasAspect);
}
...
void texture_createImage(Context* context, Texture* ptr, VkImageUsageFlags usage)
{
    ...
    texCreateInfo.format =
        formatConverter_convertToVulkan(context->deviceQueue->device->formatConverter, ptr->desc.format);
    ...
    uint32_t texMemType_device =
        context_getMemoryType(context, texMemReq.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
...
Texture* texture_create(Context* context, const pnanovdb_compute_texture_desc_t* desc)
{
    ...
    texture_createImageView(
        context, ptr, &ptr->imageViewVk_all, &ptr->imageViewVk_mipLevel, ptr->desc.format, ptr->imageAspect);
...
{
    if (a->texture_type == b->texture_type && a->usage == b->usage && a->format == b->format && a->width == b->width &&
        a->height == b->height && a->depth == b->depth && a->mip_levels == b->mip_levels)
    {
...
pnanovdb_compute_texture_t* createTexture(pnanovdb_compute_context_t* contextIn,
                                          const pnanovdb_compute_texture_desc_t* desc)
{
...
pnanovdb_compute_texture_t* createTextureExternal(pnanovdb_compute_context_t* contextIn,
                                                  const pnanovdb_compute_texture_desc_t* desc,
                                                  VkImage externalImage,
                                                  VkImageLayout defaultLayout)
{
...
void destroyTexture(pnanovdb_compute_context_t* contextIn, pnanovdb_compute_texture_t* texture)
{
    ...
    if (ptr->refCount <= 0)
    {
        context->logPrint(PNANOVDB_COMPUTE_LOG_LEVEL_ERROR,
                          "pnanovdb_compute_context_t::destroyTexture() called on inactive texture %p", texture);
    }
    ...
    if (!ptr->memoryVk && ptr->refCount <= 0)
    {
        // remove from pool
        ...
        {
            context->logPrint(PNANOVDB_COMPUTE_LOG_LEVEL_ERROR,
                              "pnanovdb_compute_context_t::destroyTexture() texture %p not found in pool %p", ptr,
                              destroy_texture);
        }
        if (destroy_texture)
        {
...
pnanovdb_compute_texture_transient_t* getTextureTransient(pnanovdb_compute_context_t* contextIn,
                                                          const pnanovdb_compute_texture_desc_t* desc)
{
    auto context = cast(contextIn);
    ...
    ptr->aliasAspect = PNANOVDB_COMPUTE_TEXTURE_ASPECT_NONE;
    ptr->nodeBegin = 0;
    ptr->nodeEnd = 0;
    return cast(ptr);
}

pnanovdb_compute_texture_transient_t* registerTextureAsTransient(pnanovdb_compute_context_t* contextIn,
...
pnanovdb_compute_texture_transient_t* aliasTextureTransient(pnanovdb_compute_context_t* contextIn,
                                                            ...
                                                            pnanovdb_compute_texture_aspect_t aspect)
{
    ...
    ptr->aliasAspect = aspect;
    ptr->nodeBegin = 0;
    ptr->nodeEnd = 0;
    return cast(ptr);
}

pnanovdb_compute_texture_acquire_t* enqueueAcquireTexture(pnanovdb_compute_context_t* contextIn,
...
pnanovdb_bool_t getAcquiredTexture(pnanovdb_compute_context_t* contextIn,
...
pnanovdb_compute_sampler_t* createSampler(pnanovdb_compute_context_t* contextIn,
