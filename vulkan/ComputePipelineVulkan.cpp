...
pnanovdb_compute_pipeline_t* createComputePipeline(pnanovdb_compute_context_t* contextIn,
                                                   const pnanovdb_compute_pipeline_desc_t* desc)
{
    ...
    ptr->pnanovdbDescriptorType_to_vkDescriptorType[PNANOVDB_COMPUTE_DESCRIPTOR_TYPE_CONSTANT_BUFFER] =
        VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    ptr->pnanovdbDescriptorType_to_vkDescriptorType[PNANOVDB_COMPUTE_DESCRIPTOR_TYPE_STRUCTURED_BUFFER] =
        VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    ptr->pnanovdbDescriptorType_to_vkDescriptorType[PNANOVDB_COMPUTE_DESCRIPTOR_TYPE_BUFFER] =
        VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER;
    ptr->pnanovdbDescriptorType_to_vkDescriptorType[PNANOVDB_COMPUTE_DESCRIPTOR_TYPE_TEXTURE] =
        VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE;
    ptr->pnanovdbDescriptorType_to_vkDescriptorType[PNANOVDB_COMPUTE_DESCRIPTOR_TYPE_SAMPLER] =
        VK_DESCRIPTOR_TYPE_SAMPLER;
    ptr->pnanovdbDescriptorType_to_vkDescriptorType[PNANOVDB_COMPUTE_DESCRIPTOR_TYPE_RW_STRUCTURED_BUFFER] =
        VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    ptr->pnanovdbDescriptorType_to_vkDescriptorType[PNANOVDB_COMPUTE_DESCRIPTOR_TYPE_RW_BUFFER] =
        VK_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER;
    ptr->pnanovdbDescriptorType_to_vkDescriptorType[PNANOVDB_COMPUTE_DESCRIPTOR_TYPE_RW_TEXTURE] =
    ...
    {
        VkDescriptorSetLayout layouts[1u] = { ptr->descriptorSetLayout };
...
void destroyComputePipeline(pnanovdb_compute_context_t* contextIn, pnanovdb_compute_pipeline_t* pipeline)
{
    ...
    context->deferredReleasePipelines.push_back(pipeline);
}
...
void computePipeline_updateDescriptorSet(Context* context,
                                         ComputePipeline* ptr,
                                         VkDescriptorSet descriptorSet,
                                         const pnanovdb_compute_dispatch_params_t* params)
{
    auto loader = &context->deviceQueue->device->loader;
    auto vulkanDevice = context->deviceQueue->device->vulkanDevice;
    ...
    for (pnanovdb_uint32_t idx = 0u; idx < params->descriptor_write_count; idx++)
    {
                 ...
                 descriptorWrite->type == PNANOVDB_COMPUTE_DESCRIPTOR_TYPE_RW_BUFFER)
        {
            ...
            VkBufferView bufferView =
                buffer_getBufferView(context, buffer, cast(resource->buffer_transient)->aliasFormat);
                 ...
                 descriptorWrite->type == PNANOVDB_COMPUTE_DESCRIPTOR_TYPE_RW_TEXTURE)
        {
            ...
            else if (descriptorWrite->type == PNANOVDB_COMPUTE_DESCRIPTOR_TYPE_TEXTURE)
            {
                ...
                imageInfo.imageView =
                    texture_getImageViewAll(context, texture, cast(resource->texture_transient)->aliasFormat,
                                            cast(resource->texture_transient)->aliasAspect);
            }
            else if (descriptorWrite->type == PNANOVDB_COMPUTE_DESCRIPTOR_TYPE_RW_TEXTURE)
            {
                Texture* texture = cast(resource->texture_transient)->texture;
                imageInfo.imageView =
                    texture_getImageViewMipLevel(context, texture, cast(resource->texture_transient)->aliasFormat,
                                                 cast(resource->texture_transient)->aliasAspect);
            }
    ...
    loader->vkUpdateDescriptorSets(
        vulkanDevice, (uint32_t)ptr->descriptorWrites.size(), ptr->descriptorWrites.data(), 0u, nullptr);
}
...
void computePipeline_dispatch(Context* context, const pnanovdb_compute_dispatch_params_t* params)
{
    ...
    {
        context->logPrint(PNANOVDB_COMPUTE_LOG_LEVEL_WARNING,
