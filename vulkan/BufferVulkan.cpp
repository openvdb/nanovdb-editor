...
void buffer_createBuffer(Context* context, Buffer* ptr, const pnanovdb_compute_interop_handle_t* interopHandle)
{
    ...
    else if (ptr->memory_type == PNANOVDB_COMPUTE_MEMORY_TYPE_READBACK)
    {
        ...
        {
            bufMemType =
                context_getMemoryType(context, bufMemReq.memoryTypeBits,
                                      VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
        }
    }
...
{
    if (a->usage == b->usage && a->format == b->format && a->size_in_bytes == b->size_in_bytes)
    {
...
pnanovdb_compute_buffer_t* createBuffer(pnanovdb_compute_context_t* contextIn,
                                        pnanovdb_compute_memory_type_t memory_type,
                                        const pnanovdb_compute_buffer_desc_t* desc)
{
...
pnanovdb_compute_buffer_t* createBufferFromExternalHandle(pnanovdb_compute_context_t* contextIn,
                                                          const pnanovdb_compute_buffer_desc_t* desc,
                                                          const pnanovdb_compute_interop_handle_t* interopHandle)
{
    auto context = cast(contextIn);
...
void destroyBuffer(pnanovdb_compute_context_t* contextIn, pnanovdb_compute_buffer_t* buffer)
{
    ...
    if (ptr->refCount <= 0)
    {
        context->logPrint(PNANOVDB_COMPUTE_LOG_LEVEL_ERROR,
                          "pnanovdb_compute_context_t::destroyBuffer() called on inactive buffer %p", buffer);
        context->logPrint(PNANOVDB_COMPUTE_LOG_LEVEL_ERROR,
                          "buffer_desc_t usage(%d) format(%d) structure_stride(%d) size_in_bytes(%zu) memory_type(%d)",
                          ptr->desc.usage, ptr->desc.format, ptr->desc.structure_stride, ptr->desc.size_in_bytes,
                          ptr->memory_type);
    }
...
pnanovdb_compute_buffer_transient_t* getBufferTransient(pnanovdb_compute_context_t* contextIn,
                                                        const pnanovdb_compute_buffer_desc_t* desc)
{
    auto context = cast(contextIn);
...
pnanovdb_compute_buffer_transient_t* registerBufferAsTransient(pnanovdb_compute_context_t* contextIn,
...
pnanovdb_compute_buffer_transient_t* aliasBufferTransient(pnanovdb_compute_context_t* contextIn,
...
pnanovdb_compute_buffer_acquire_t* enqueueAcquireBuffer(pnanovdb_compute_context_t* contextIn,
                                                        pnanovdb_compute_buffer_transient_t* bufferTransient)
{
    auto context = cast(contextIn);
...
pnanovdb_bool_t getAcquiredBuffer(pnanovdb_compute_context_t* contextIn,
                                    ...
                                    pnanovdb_uint64_t* pBufferSizeInBytes)
{
    ...
    ctx->deviceQueue->device->loader.vkGetMemoryWin32HandleKHR(
        ctx->deviceQueue->device->vulkanDevice, &handleInfo, &handle);
...
#endif
    device_getBufferExternalHandle(
        context, buffer, &dstHandle->value, sizeof(dstHandle->value), &dstHandle->resource_size_in_bytes);
}
