...
#    include <dlfcn.h> // Required for dladdr
#endif
...
#    if defined(_WIN32)
    const char* vulkanPath = "vulkan-1.dll";
#    else
    char vulkanPathBuf[1024] = { 0 };
    const char* vulkanPath = vulkanPathBuf;
    Dl_info selfInfo;
    if (dladdr((void*)&createDeviceManager, &selfInfo) && selfInfo.dli_fname)
    {
        strncpy(vulkanPathBuf, selfInfo.dli_fname, sizeof(vulkanPathBuf) - 1);
        vulkanPathBuf[sizeof(vulkanPathBuf) - 1] = '\0';
        char* lastSlash = strrchr(vulkanPathBuf, '/');
        if (lastSlash)
        {
            *(lastSlash + 1) = '\0';
        }
...
#        if defined(__APPLE__)
    strncat(vulkanPathBuf, "libvulkan.1.dylib", sizeof(vulkanPathBuf) - strlen(vulkanPathBuf) - 1);
#        else
    strncat(vulkanPathBuf, "libvulkan.so.1", sizeof(vulkanPathBuf) - strlen(vulkanPathBuf) - 1);
#        endif
#    endif

    ptr->vulkan_module = pnanovdb_load_library(vulkanPath, vulkanPath, vulkanPath);
#else
    ptr->vulkan_module = pnanovdb_load_library("vulkan-1.dll", "libvulkan.so.1", "libvulkan.1.dylib");
#endif // PNANOVDB_USE_GLFW

    if (!ptr->vulkan_module)
    {
#if defined(_WIN32)
        printf("Error: Vulkan library failed to load\n");
#else
        printf("Error: Vulkan library failed to load: %s\n", dlerror());
#endif
        return nullptr;
    }
    /*
        if (ptr->vulkan_module)
        {
            char vulkan_lib_path[1024];
    #if defined(_WIN32)
            HMODULE vulkan_module_handle = (HMODULE)ptr->vulkan_module;
            if (GetModuleFileNameA(vulkan_module_handle, vulkan_lib_path, sizeof(vulkan_lib_path)) > 0)
            {
            }
    #else
            Dl_info info;
            void* getInstanceProcAddr = pnanovdb_get_proc_address(ptr->vulkan_module, "vkGetInstanceProcAddr");
            if (getInstanceProcAddr && dladdr(getInstanceProcAddr, &info) && info.dli_fname)
            {
                strncpy(vulkan_lib_path, info.dli_fname, sizeof(vulkan_lib_path) - 1);
                vulkan_lib_path[sizeof(vulkan_lib_path) - 1] = '\0';
            }
    #endif
            printf("Vulkan library path: %s\n", vulkan_lib_path);
        }
    */
    ...
    if (loader->vkEnumerateInstanceVersion)
    {
        ...
        loader->vkEnumerateInstanceVersion(&api_version);
        ptr->vulkanInstanceVersion = api_version;

        uint32_t min_video_api_version = VK_MAKE_VERSION(1, 3, 275);
        if (api_version >= min_video_api_version)
        {
            ptr->instanceSupportsVulkanVideo = PNANOVDB_TRUE;
        }
    }
    ...
    appInfo.pApplicationName = "NanoVDB Editor";
    appInfo.applicationVersion = VK_MAKE_VERSION(1, 0, 0);
    appInfo.pEngineName = "NanoVDB";
                ...
                ptr->enabledExtensions.VK_KHR_EXTERNAL_FENCE_CAPABILITIES)
            {
                ...
                pnanovdb_uint64_t uuidNumBytes =
                    sizeof(deviceIdVulkan.deviceUUID) < sizeof(physicalDeviceDesc.device_uuid) ?
                        ...
                        sizeof(physicalDeviceDesc.device_uuid);
                ...
                pnanovdb_uint64_t luidNumBytes =
                    sizeof(deviceIdVulkan.deviceLUID) < sizeof(physicalDeviceDesc.device_luid) ?
                        sizeof(deviceIdVulkan.deviceLUID) :
                        sizeof(physicalDeviceDesc.device_luid);
...
pnanovdb_bool_t enumerateDevices(pnanovdb_compute_device_manager_t* manager,
                                 pnanovdb_uint32_t deviceIndex,
                                 pnanovdb_compute_physical_device_desc_t* pDesc)
{
...
pnanovdb_compute_device_t* createDevice(pnanovdb_compute_device_manager_t* deviceManagerIn,
                                        const pnanovdb_compute_device_desc_t* desc)
{
    auto deviceManager = cast(deviceManagerIn);

    if (deviceManager == nullptr || desc == nullptr)
    {
        return nullptr;
    }
    ...
    selectDeviceExtensions(ptr, deviceExtensionsEnabled, deviceManager->instanceSupportsVulkanVideo);

    if (!deviceManager->instanceSupportsVulkanVideo)
    {
        ptr->logPrint(PNANOVDB_COMPUTE_LOG_LEVEL_WARNING,
                      "Vulkan Instance Version %d.%d.%d is too old to support Vulkan Video. At least 1.3.275 is required",
                      VK_API_VERSION_MAJOR(deviceManager->vulkanInstanceVersion),
                      VK_API_VERSION_MINOR(deviceManager->vulkanInstanceVersion),
                      VK_API_VERSION_PATCH(deviceManager->vulkanInstanceVersion));
        ptr->logPrint(PNANOVDB_COMPUTE_LOG_LEVEL_WARNING,
                      "Please upgrade Vulkan install as described in the NanoVDB Editor readme.");
    }
    ...
    // identify graphics and compute queues
    {
        ...
        if (computeQueueFamilyIdx == ~0u)
        {
            for (uint32_t i = 0u; i < queueCount; i++)
            {
                if (i != ptr->graphicsQueueFamilyIdx && queueProps[i].queueFlags & VK_QUEUE_COMPUTE_BIT)
                {
                    computeQueueFamilyIdx = i;
                    break;
                }
        ...
        uint32_t encodeQueueFamilyIdx = ~0u;
        if (ptr->enabledExtensions.VK_KHR_VIDEO_QUEUE && ptr->enabledExtensions.VK_KHR_VIDEO_ENCODE_QUEUE)
        {
            for (uint32_t i = 0u; i < queueCount; i++)
            {
                if (queueProps[i].queueFlags & VK_QUEUE_VIDEO_ENCODE_BIT_KHR)
                {
                    encodeQueueFamilyIdx = i;
                    break;
                }
            }
        }
        ptr->encodeQueueFamilyIdx = encodeQueueFamilyIdx;
    ...
    // create device
    {
        ...
        else
        {
            ...
            queueCreateInfo[queueCreateInfoCount].queueFamilyIndex = ptr->computeQueueFamilyIdx;
            queueCreateInfo[queueCreateInfoCount].queueCount = 1u;
            queueCreateInfo[queueCreateInfoCount].pQueuePriorities = queuePriorities;
            queueCreateInfoCount++;
        }
        if (ptr->encodeQueueFamilyIdx != ~0u)
        {
            queueCreateInfo[queueCreateInfoCount].sType = VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO;
            queueCreateInfo[queueCreateInfoCount].queueFamilyIndex = ptr->encodeQueueFamilyIdx;
            queueCreateInfo[queueCreateInfoCount].queueCount = 1u;
            queueCreateInfo[queueCreateInfoCount].pQueuePriorities = queuePriorities;
            queueCreateInfoCount++;
        }
...
#undef PNANOVDB_VULKAN_TRY_ENABLE_FEATURE
    ...
    if (ptr->encodeQueueFamilyIdx != ~0u)
    {
        deviceLoader->vkGetDeviceQueue(ptr->vulkanDevice, ptr->encodeQueueFamilyIdx, 0u, &ptr->encodeQueueVk);
    }
...
void destroyDevice(pnanovdb_compute_device_manager_t* manager, pnanovdb_compute_device_t* device)
{
    ...
    deviceQueue_destroy(ptr->computeQueue);
...
pnanovdb_compute_semaphore_t* createSemaphore(pnanovdb_compute_device_t* device)
{
    ...
    ptr->device->loader.vkCreateSemaphore(ptr->device->vulkanDevice, &semaphoreCreateInfo, nullptr, &ptr->semaphoreVk);
...
void destroySemaphore(pnanovdb_compute_semaphore_t* semaphore)
{
    ...
    ptr->device->loader.vkDestroySemaphore(ptr->device->vulkanDevice, ptr->semaphoreVk, nullptr);
...
void closeSemaphoreExternalHandle(pnanovdb_compute_semaphore_t* semaphore,
                                  const void* srcHandle,
                                  pnanovdb_uint64_t srcHandleSize)
{
    auto ptr = cast(semaphore);
#if defined(_WIN32)
    HANDLE handle = {};
...
DeviceQueue* deviceQueue_create(Device* device, uint32_t queueFamilyIdx, VkQueue queue)
{
    ...
    for (int i = 0; i < kMaxFramesInFlight; i++)
    {
        loader->vkCreateSemaphore(ptr->vulkanDevice, &semaphoreCreateInfo, nullptr, &ptr->beginFrameSemaphore[i]);
        loader->vkCreateSemaphore(ptr->vulkanDevice, &semaphoreCreateInfo, nullptr, &ptr->endFrameSemaphore[i]);
    }
...
void deviceQueue_destroy(DeviceQueue* ptr)
{
    ...
    for (int i = 0; i < kMaxFramesInFlight; i++)
    {
        loader->vkDestroySemaphore(ptr->vulkanDevice, ptr->beginFrameSemaphore[i], nullptr);
        loader->vkDestroySemaphore(ptr->vulkanDevice, ptr->endFrameSemaphore[i], nullptr);
    }
...
int flush(pnanovdb_compute_queue_t* deviceQueue,
...
pnanovdb_compute_swapchain_t* createSwapchain(pnanovdb_compute_queue_t* queue,
                                              const pnanovdb_compute_swapchain_desc_t* desc)
{
    ...
    instanceLoader->vkGetPhysicalDeviceSurfaceFormatsKHR(
        device->physicalDevice, ptr->surfaceVulkan, &surfaceCount, nullptr);
    if (surfaceCount > 32u)
        surfaceCount = 32u;
    instanceLoader->vkGetPhysicalDeviceSurfaceFormatsKHR(
        device->physicalDevice, ptr->surfaceVulkan, &surfaceCount, surfaceFormats);
...
void swapchain_initSwapchain(Swapchain* ptr)
{
    ...
    instanceLoader->vkGetPhysicalDeviceSurfacePresentModesKHR(
        device->physicalDevice, ptr->surfaceVulkan, &numPresentModes, nullptr);
    if (numPresentModes > 8u)
        numPresentModes = 8u;
    instanceLoader->vkGetPhysicalDeviceSurfacePresentModesKHR(
        device->physicalDevice, ptr->surfaceVulkan, &numPresentModes, presentModes);

    VkPresentModeKHR presentOptions[3u] = { VK_PRESENT_MODE_IMMEDIATE_KHR, VK_PRESENT_MODE_MAILBOX_KHR,
    ...
    VkResult result =
        deviceLoader->vkCreateSwapchainKHR(device->vulkanDevice, &swapchainDesc, nullptr, &ptr->swapchainVulkan);
    ...
    deviceLoader->vkGetSwapchainImagesKHR(
        device->vulkanDevice, ptr->swapchainVulkan, &ptr->numSwapchainImages, ptr->swapchainImages);
    ...
    for (pnanovdb_uint32_t idx = 0; idx < ptr->numSwapchainImages; idx++)
    {
        ...
        ptr->textures[idx] = createTextureExternal(
            cast(ptr->deviceQueue->context), &texDesc, ptr->swapchainImages[idx], VK_IMAGE_LAYOUT_PRESENT_SRC_KHR);
    }
...
int presentSwapchain(pnanovdb_compute_swapchain_t* swapchain, pnanovdb_bool_t vsync, pnanovdb_uint64_t* flushedFrameID)
{
    ...
    *flushedFrameID = ptr->deviceQueue->nextFenceValue;
    ...
    ptr->deviceQueue->currentEndFrameSemaphore = ptr->deviceQueue->endFrameSemaphore[ptr->deviceQueue->commandBufferIdx];
...
pnanovdb_compute_texture_t* getSwapchainFrontTexture(pnanovdb_compute_swapchain_t* swapchain)
{
    ...
    ptr->deviceQueue->currentBeginFrameSemaphore =
        ptr->deviceQueue->beginFrameSemaphore[ptr->deviceQueue->commandBufferIdx];

    VkResult result = loader->vkAcquireNextImageKHR(device->vulkanDevice, ptr->swapchainVulkan, UINT64_MAX,
