...
void context_destroy(Context* ptr)
{
    ...
    context_destroyComputePipelines(ptr);
...
void addPassCopyBuffer(pnanovdb_compute_context_t* contextIn, const pnanovdb_compute_copy_buffer_params_t* params)
{
    ...
    node->descriptorWrites.push_back(
        pnanovdb_compute_descriptor_write_t{ PNANOVDB_COMPUTE_DESCRIPTOR_TYPE_BUFFER_COPY_SRC });
    node->resources.push_back(src);
    node->descriptorWrites.push_back(
        pnanovdb_compute_descriptor_write_t{ PNANOVDB_COMPUTE_DESCRIPTOR_TYPE_BUFFER_COPY_DST });
...
void context_flushNodes(Context* context)
{
    ...
    for (pnanovdb_uint32_t nodeIdx = 0u; nodeIdx < context->nodes.size(); nodeIdx++)
    {
        ...
        for (pnanovdb_uint32_t idx = 0u; idx < node->bufferTransientsCreate.size(); idx++)
        {
            ...
            else
            {
                ...
                {
                    context->logPrint(
                        PNANOVDB_COMPUTE_LOG_LEVEL_ERROR, "pnanovdb_compute_context_t::BufferTransient double create");
                }
                transient->buffer =
                    cast(createBuffer(cast(context), PNANOVDB_COMPUTE_MEMORY_TYPE_DEVICE, &transient->desc));
            }
        }
        for (pnanovdb_uint32_t idx = 0u; idx < node->textureTransientsCreate.size(); idx++)
        {
            ...
            else
            {
                ...
                {
                    context->logPrint(
                        PNANOVDB_COMPUTE_LOG_LEVEL_ERROR, "pnanovdb_compute_context_t::TextureTransient double create");
                }
    ...
    for (pnanovdb_uint32_t nodeIdx = 0u; nodeIdx < context->nodes.size(); nodeIdx++)
    {
        ...
        if (node->bufferBarriers.size() > 0u || node->imageBarriers.size() > 0u)
        {
            loader->vkCmdPipelineBarrier(context->deviceQueue->commandBuffer, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
                                         VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0, 0u, nullptr,
    ...
    if (context->restore_bufferBarriers.size() > 0u || context->restore_imageBarriers.size() > 0u)
    {
        loader->vkCmdPipelineBarrier(
            context->deviceQueue->commandBuffer, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
            0, 0u, nullptr, (uint32_t)context->restore_bufferBarriers.size(), context->restore_bufferBarriers.data(),
    ...
    // global barrier
    {
        ...
        loader->vkCmdPipelineBarrier(context->deviceQueue->commandBuffer, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
                                     VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0, 3u, barriers, 0u, nullptr, 0u, nullptr);
    }
    ...
    for (pnanovdb_uint32_t idx = context->deferredReleaseBuffers.size() - 1u;
         idx < context->deferredReleaseBuffers.size(); idx--)
    {
        auto ptr = cast(context->deferredReleaseBuffers[idx]);
        if (ptr)
        {
            // Only process UPLOAD buffers if lastActive <= lastFenceCompleted
            ...
            ptr->refCount--;
            ptr->lastActive = ptr->memory_type != PNANOVDB_COMPUTE_MEMORY_TYPE_DEVICE ?
                                  context->deviceQueue->lastFenceCompleted :
                                  context->deviceQueue->nextFenceValue;
    ...
    for (pnanovdb_uint32_t idx = context->deferredReleaseTextures.size() - 1u;
         idx < context->deferredReleaseTextures.size(); idx--)
    {
    ...
    // apply deferred destroyPipeline
    for (pnanovdb_uint32_t idx = context->deferredReleasePipelines.size() - 1u;
         idx < context->deferredReleasePipelines.size(); idx--)
    {
        auto ptr = cast(context->deferredReleasePipelines[idx]);
        if (ptr->lastActive <= context->deviceQueue->lastFenceCompleted)
        {
            // context->logPrint(PNANOVDB_COMPUTE_LOG_LEVEL_INFO, "Doing pipeline release");
            computePipeline_destroy(context, context->deferredReleasePipelines[idx]);
            context->deferredReleasePipelines.erase(context->deferredReleasePipelines.begin() + idx);
        }
    }
...
void profilerCapture_init(Context* context, ProfilerCapture* ptr, pnanovdb_uint64_t capacity)
{
    ...
    uint32_t bufMemType = context_getMemoryType(
        context, bufMemReq.memoryTypeBits,
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT | VK_MEMORY_PROPERTY_HOST_CACHED_BIT);
    if (bufMemType == ~0u)
    {
        bufMemType = context_getMemoryType(context, bufMemReq.memoryTypeBits,
...
void profilerCapture_reset(Context* context, ProfilerCapture* ptr, pnanovdb_uint64_t minCapacity, pnanovdb_uint64_t captureID)
{
    if (ptr->state == 0u)
    {
        ...
        ptr->queryFrequency =
            (pnanovdb_uint64_t)(double(1.0E9) /
                                double(context->deviceQueue->device->physicalDeviceProperties.limits.timestampPeriod));
...
void profilerCapture_timestamp(Context* context, ProfilerCapture* ptr, const char* label)
{
    if (ptr->state == 1u && ptr->entries.size() < ptr->capacity)
    {
        ...
        loader->vkCmdWriteTimestamp(
            context->deviceQueue->commandBuffer, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, ptr->queryPool, entry_idx);
...
void profilerCapture_download(Context* context, ProfilerCapture* ptr)
{
    if (ptr->state == 1u)
    {
        ...
        loader->vkCmdCopyQueryPoolResults(context->deviceQueue->commandBuffer, ptr->queryPool, 0u,
                                          (uint32_t)ptr->entries.size(), ptr->queryBuffer, 0llu,
                                          sizeof(pnanovdb_uint64_t), VK_QUERY_RESULT_64_BIT | VK_QUERY_RESULT_WAIT_BIT);
                                           ...
                                           pnanovdb_compute_profiler_entry_t** pEntries)
{
    if (ptr->state == 2u)
    {
        ...
        for (pnanovdb_uint32_t idx = 0u; idx < ptr->entries.size(); idx++)
        {
            ...
            deltaEntry->cpu_delta_time =
                (float)(((double)(entry->cpuValue - prevEntry.cpuValue) / (double)(ptr->cpuFreq)));
            deltaEntry->gpu_delta_time =
                (float)(((double)(entry->gpuValue - prevEntry.gpuValue) / (double)(ptr->queryFrequency)));
...
void enableProfiler(pnanovdb_compute_context_t* contextIn,
                    void* userdata,
