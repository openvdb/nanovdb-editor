...
/*!
    \file   nanovdb_editor/editor/Node2Convert.cpp
...
static void node2_profiler_report(void* userdata,
...
int node2_verify_gpu(const char* nvdb_ref_filepath, const char* nvdb_filepath)
{
    ...
    compute.dispatch_shader_on_array(&compute, device, "editor/node2_verify_gpu.slang", params.grid_dim_x,
                                     params.grid_dim_y, 1u, data_in, constants, data_out, 8u, 0llu, 0llu);
    ...
    compute.dispatch_shader_on_array(&compute, device, "editor/node2_verify_gpu_ref.slang", params.grid_dim_x,
                                     params.grid_dim_y, 1u, data_in_ref, constants, data_out_ref, 8u, 0llu, 0llu);

    compute.device_interface.destroy_device(device_manager, device);
    device = compute.device_interface.create_device(device_manager, &device_desc);

    printf("Benchmark for OnIndex NanoVDB:\n");
    compute.dispatch_shader_on_array(&compute, device, "editor/node2_verify_gpu_onindex.slang", params.grid_dim_x,
                                     params.grid_dim_y, 1u, data_in_onindex, constants, data_out_onindex, 8u, 0llu, 0llu);
            ...
            for (pnanovdb_int32_t k = bbox_min.z; k < bbox_max.z; k++)
            {
                ...
                for (int i = 0; i < PNANOVDB_NODE2_TEST_X_DEPTH; i++)
                {
                    ...
                    pnanovdb_uint64_t value_idx =
                        pnanovdb_node2_get_value_index(buf, node, node_type, node_n, PNANOVDB_TRUE,
                                                       values.node_inactive_idxs, values.inactive_value_idxs);
                    ...
                    uint64_t cpp_val_idx =
                        nanovdb::node2_get_value_index(buf_64, root_idx64, nanovdb::Coord(ijk.x, ijk.y, ijk.z), true,
                                                       uint32_t(values.node_inactive_idxs.byte_offset >> 3u),
                                                       uint32_t(values.inactive_value_idxs.byte_offset >> 3u));
                ...
                if (val_cpu != val_gpu || val_gpu != val_gpu_ref || val_cpp != val_gpu)
                {
                    mismatched_count++;
                    if (print_count < 32u)
                    {
                        ...
                        pnanovdb_editor::Console::getInstance().addLog(
                            "val_cpu(%f) val_gpu(%f) val_gpu_ref(%f) ijk(%d,%d,%d) node_type(0x%x) dense_n(%d)",
                            val_cpu, val_gpu, val_gpu_ref, i, j, k, node_type, node_n);
                        printf(
                            "element[%llu] val_cpu(%f) val_cpp(%f) val_gpu(%f) val_gpu_ref(%f) ijk(%d,%d,%d) node_type(0x%x) dense_n(%d)\n",
                            (unsigned long long int)idx, val_cpu, val_cpp, val_gpu, val_gpu_ref, i, j, k, node_type,
                            node_n);
                    }
                }
    ...
    pnanovdb_editor::Console::getInstance().addLog("node2_verify_gpu() mismatched(%lld) total_count(%lld)",
                                                   (unsigned long long int)mismatched_count,
    ...
    pnanovdb_editor::Console::getInstance().addLog("node2_verify_gpu() mismatched_onindex(%lld) total_count(%lld)",
                                                   (unsigned long long int)mismatched_count_onindex,
                                                   (unsigned long long int)total_count);
