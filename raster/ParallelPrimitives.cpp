...
static void global_scan_generic(const pnanovdb_compute_t* compute,
                                pnanovdb_compute_queue_t* queue,
                                pnanovdb_parallel_primitives_context_t* context_in,
                                ...
                                pnanovdb_uint32_t dispatch_count)
{
    ...
    for (pnanovdb_uint32_t dispatch_idx = 0u; dispatch_idx < dispatch_count; dispatch_idx++)
    {
        // scan 1
        {
            ...
            compute->dispatch_shader(compute_interface, context, ctx->shader_ctx[scan1_generic], resources,
                                     workgroup_count, 1u, 1u, "scan1");
        }
        // scan 2
        {
            pnanovdb_compute_resource_t resources[3u] = {};
            ...
            compute->dispatch_shader(
                compute_interface, context, ctx->shader_ctx[scan2_generic], resources, 1u, 1u, 1u, "scan2");
        }
        // scan 3
        {
            ...
            compute->dispatch_shader(compute_interface, context, ctx->shader_ctx[scan3_generic], resources,
                                     workgroup_count, 1u, 1u, "scan3");
        }
    }
...
static void global_scan(const pnanovdb_compute_t* compute,
                        pnanovdb_compute_queue_t* queue,
                        pnanovdb_parallel_primitives_context_t* context_in,
                        pnanovdb_compute_buffer_t* val_in,
                        pnanovdb_compute_buffer_t* val_out,
...
{
    global_scan_generic(
        compute, queue, context_in, val_in, val_out, val_count, PNANOVDB_FALSE, PNANOVDB_FALSE, dispatch_count);
}

static void global_scan_uint64(const pnanovdb_compute_t* compute,
                               pnanovdb_compute_queue_t* queue,
                               pnanovdb_parallel_primitives_context_t* context_in,
                               pnanovdb_compute_buffer_t* val_in,
                               pnanovdb_compute_buffer_t* val_out,
...
{
    global_scan_generic(
        compute, queue, context_in, val_in, val_out, val_count, PNANOVDB_TRUE, PNANOVDB_FALSE, dispatch_count);
}

static void global_scan_max(const pnanovdb_compute_t* compute,
                            pnanovdb_compute_queue_t* queue,
                            pnanovdb_parallel_primitives_context_t* context_in,
                            pnanovdb_compute_buffer_t* val_in,
                            pnanovdb_compute_buffer_t* val_out,
...
{
    global_scan_generic(
        compute, queue, context_in, val_in, val_out, val_count, PNANOVDB_FALSE, PNANOVDB_TRUE, dispatch_count);
}

static int global_scan_array(const pnanovdb_compute_t* compute,
                             pnanovdb_compute_queue_t* queue,
                             pnanovdb_parallel_primitives_context_t* context_in,
                             ...
                             pnanovdb_uint32_t dispatch_count)
{
    ...
    global_scan(compute, queue, context_in, val_in_gpu_array->device_buffer, val_out_gpu_array->device_buffer,
                val_count, dispatch_count);
...
static void radix_sort(const pnanovdb_compute_t* compute,
                       pnanovdb_compute_queue_t* queue,
                       pnanovdb_parallel_primitives_context_t* context_in,
                       ...
                       pnanovdb_uint32_t key_bit_count)
{
    ...
    for (pnanovdb_uint32_t pass_id = 0u; pass_id < pass_count; pass_id++)
    {
        ...
        // radix sort 2
        {
            ...
            compute->dispatch_shader(
                compute_interface, context, ctx->shader_ctx[radix_sort2_slang], resources, 2u, 1u, 1u, "radix_sort2");
        }
        // radix sort 3
        {
...
static void radix_sort_dual_key(const pnanovdb_compute_t* compute,
                                pnanovdb_compute_queue_t* queue,
                                pnanovdb_parallel_primitives_context_t* context_in,
                                ...
                                pnanovdb_uint32_t key_high_bit_count)
{
    ...
    radix_sort(
        compute, queue, context_in, key_tmp_buffer, val_tmp_buffer, key_count, buffer_key_count, key_low_bit_count);
    ...
    radix_sort(
        compute, queue, context_in, key_tmp_buffer, val_tmp_buffer, key_count, buffer_key_count, key_high_bit_count);
