...
/*!
    \file   nanovdb_editor/raster/Node2Cpp.h
...
uint32_t node2_adaptive_n(uint64_t* buf, uint32_t node, uint32_t node_type, uint32_t node_n)
{
    ...
    uint32_t level_idx = ((node_n & node2_level_mask_i[node_type]) >> 3) |
                         ((node_n & node2_level_mask_j[node_type]) >> 2) |
                         ((node_n & node2_level_mask_k[node_type]) >> 1);
    // fetch 4-bit level
    uint32_t level = uint32_t(buf[node + node2_off_value_level + (level_idx >> 4u)] >> ((level_idx << 2u) & 63u)) & 15;
    // clear bits to apply level
    uint32_t level_mask = (1u << level) - 1u;
    uint32_t level_mask_3d =
        (level_mask << node2_fanout_bits2[node_type]) | (level_mask << node2_fanout_bits[node_type]) | level_mask;
...
void node2_find_node_pass(uint64_t* buf,
                          uint32_t node,
                          int32_t root_n_off,
                          Coord ijk,
                          uint32_t* out_node,
                          uint32_t* out_node_type,
                          uint32_t* out_node_n,
                          bool* out_key_is_match)
{
    *out_key_is_match = true;
    ...
    for (; node_type < 4u; node_type++)
    {
        ...
        if (node_type == 0u)
        {
            node_n += root_n_off;
            // verify key match
            uint64_t key = uint64_t(uint32_t(ijk.z()) >> 12) | (uint64_t(uint32_t(ijk.y()) >> 12) << 21) |
                           (uint64_t(uint32_t(ijk.x()) >> 12) << 42);
            uint32_t child_count = uint32_t(buf[node + node2_off_child_mask_prefix_sum[node_type]]) & 0xFFFF;
            bool key_active;
            uint32_t key_idx = node2_mask_n_to_idx(
                buf, node + node2_off_key_mask_prefix_sum[node_type], node_type, node_n, &key_active);
        ...
        bool is_active;
        uint32_t child_idx =
            node2_mask_n_to_idx(buf, node + node2_off_child_mask_prefix_sum[node_type], node_type, node_n, &is_active);
        uint32_t child = 0u;
        if (is_active)
        {
...
void node2_find_node(uint64_t* buf, uint32_t root, Coord ijk, uint32_t* out_node, uint32_t* out_node_type, uint32_t* out_node_n)
{
    ...
    if (!key_is_match)
    {
        uint32_t node_n = (((ijk.x() & node2_ijk_mask[0u]) >> node2_tiledim_bits[0u]) << node2_fanout_bits2[0u]) |
...
uint64_t node2_get_value_index(uint64_t* buf,
                               uint32_t node,
                               Coord ijk,
                               bool is_levelset,
                               uint32_t first_node_inactive_idx,
                               uint32_t first_inactive_value_idx)
{
    ...
    bool is_active;
    uint32_t local_value_idx =
        node2_mask_n_to_idx(buf, node + node2_off_value_mask_prefix_sum[node_type], node_type, node_n, &is_active);
