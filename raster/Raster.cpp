...
static int point_frag_alloc(const pnanovdb_compute_t* compute,
                            ...
                            pnanovdb_uint32_t dispatch_count)
{
    ...
    // positions to ijk
    {
...

        {
            ...
            compute->dispatch_shader(compute_interface, context, ctx->shader_ctx[point_frag_alloc_slang], resources,
                                     (constants.point_count + 255u) / 256u, 1u, 1u, "point_to_ijks");
        }
...
static pnanovdb_compute_buffer_t* raster_get_constant_buffer(const pnanovdb_compute_t* compute,
                                                             pnanovdb_compute_queue_t* queue,
...
static int gaussian_prim(const pnanovdb_compute_t* compute,
                         pnanovdb_compute_queue_t* queue,
                         raster_context_t* ctx,
                         pnanovdb_grid_build_fanout_state_t* state,
                         pnanovdb_compute_buffer_t* positions_in,
                         ...
                         pnanovdb_uint32_t dispatch_count)
{
    ...
    // gaussian prim shader
    {
        ...
        compute->dispatch_shader(compute_interface, context, ctx->shader_ctx[gaussian_prim_slang], resources,
                                 (constants.prim_batch_size + 255u) / 256u, 1u, 1u, "gaussian_prim");
    }
...
static int gaussian_frag_alloc(const pnanovdb_compute_t* compute,
                               pnanovdb_compute_queue_t* queue,
                               raster_context_t* ctx,
                               pnanovdb_grid_build_fanout_state_t* state,
                               pnanovdb_compute_buffer_t* positions_in,
                               ...
                               pnanovdb_uint32_t dispatch_count)
{
    ...
    // gaussian frag shader
    {
        ...
        compute->dispatch_shader(compute_interface, context, ctx->shader_ctx[gaussian_frag_alloc_slang], resources,
                                 (constants.ijk_batch_size + 255u) / 256u, 1u, 1u, "gaussian_frag_alloc");
    }
...
static int gaussian_frag_color(const pnanovdb_compute_t* compute,
                               pnanovdb_compute_queue_t* queue,
                               raster_context_t* ctx,
                               pnanovdb_grid_build_fanout_state_t* state,
                               pnanovdb_compute_buffer_t* positions_in,
                               ...
                               pnanovdb_compute_buffer_t* shader_params_in,
                               ...
                               pnanovdb_uint32_t dispatch_count)
{
    ...
    pnanovdb_compute_buffer_transient_t* shader_params_transient =
        compute_interface->register_buffer_as_transient(context, shader_params_in);
    ...
    // gaussian frag shader
    {
        ...
        resources[3u].buffer_transient = quaternions_transient;
        resources[4u].buffer_transient = scales_transient;
        resources[5u].buffer_transient = opacities_transient;
        resources[6u].buffer_transient = colors_transient;
        resources[7u].buffer_transient = prim_idxs_transient;
        resources[8u].buffer_transient = prim_raster_idxs_transient;
        resources[9u].buffer_transient = prim_bbox_transient;
        resources[10u].buffer_transient = nanovdb_transient;

        compute->dispatch_shader(compute_interface, context, ctx->shader_ctx[gaussian_frag_color_slang], resources,
                                 (constants.ijk_batch_size + 255u) / 256u, 1u, 1u, "gaussian_frag_color");
    }
...
static int point_frag_color(const pnanovdb_compute_t* compute,
                            pnanovdb_compute_queue_t* queue,
                            raster_context_t* ctx,
                            pnanovdb_compute_buffer_t* colors_in,
                            ...
                            pnanovdb_uint32_t dispatch_count)
{
    ...
    // splat colors
    {
...

        {
            ...
            compute->dispatch_shader(compute_interface, context, ctx->shader_ctx[point_frag_color_slang], resources,
                                     (constants.point_count + 255u) / 256u, 1u, 1u, "point_frag_color");
        }
                        ...
                        void* userdata)
{
    ...
    // init loop state
    pnanovdb_grid_build_fanout_state_t state = {};
    ctx->grid_build.fanout_state_init(compute, queue, ctx->grid_build_ctx, &state, prim_batch_size, ijk_batch_size,
                                      ijk_batch_count_max, data->point_count, voxel_size);

    pnanovdb_grid_build_state_t grid_state = {};
    ctx->grid_build.grid_build_init(
        compute, queue, ctx->grid_build_ctx, &grid_state, nanovdb_out, nanovdb_word_count, voxel_size, dispatch_count);
    ...
    for (pnanovdb_uint32_t raster_idx = 0u; raster_idx < raster_count; raster_idx++)
    {
        ...
        // init loop state
        ctx->grid_build.fanout_state_reset(compute, queue, ctx->grid_build_ctx, &state);

        ctx->grid_build.grid_build_reset(compute, queue, ctx->grid_build_ctx, &grid_state, nanovdb_out,
                                         nanovdb_word_count, voxel_size, dispatch_count);

        while (ctx->grid_build.fanout_state_valid(compute, queue, ctx->grid_build_ctx, &state))
        {
            // gaussian prim shader
            gaussian_prim(compute, queue, ctx, &state, data->means_gpu_array->device_buffer,
                          data->quaternions_gpu_array->device_buffer, data->scales_gpu_array->device_buffer,
                          data->opacities_gpu_array->device_buffer, dispatch_count);

            // raster fanout
            ctx->grid_build.fanout(compute, queue, ctx->grid_build_ctx, &state, dispatch_count);

            // gaussian frag alloc
            gaussian_frag_alloc(compute, queue, ctx, &state, data->means_gpu_array->device_buffer,
                                data->quaternions_gpu_array->device_buffer, data->scales_gpu_array->device_buffer,
                                data->opacities_gpu_array->device_buffer, state.prim_raster_ijks_buffer, 1u);

            ctx->grid_build.grid_build(compute, queue, ctx->grid_build_ctx, &grid_state, state.prim_raster_ijks_buffer,
                                       nanovdb_out, state.ijk_batch_size, nanovdb_word_count, voxel_size, dispatch_count);

            pnanovdb_uint64_t flushed_frame = 0llu;
            compute->device_interface.flush(queue, &flushed_frame, nullptr, nullptr);

            ctx->grid_build.fanout_state_increment(compute, queue, ctx->grid_build_ctx, &state);
        }

        ctx->grid_build.grid_build_finalize(compute, queue, ctx->grid_build_ctx, &grid_state,
                                            state.prim_raster_ijks_buffer, nanovdb_out, state.ijk_batch_size,
                                            nanovdb_word_count, voxel_size, dispatch_count);
        ...
        while (ctx->grid_build.fanout_state_valid(compute, queue, ctx->grid_build_ctx, &state))
        {
            ...
            // gaussian frag color
            gaussian_frag_color(compute, queue, ctx, &state, data->means_gpu_array->device_buffer,
                                data->quaternions_gpu_array->device_buffer, data->scales_gpu_array->device_buffer,
                                data->opacities_gpu_array->device_buffer, data->colors_gpu_array->device_buffer,
                                data->shader_params_gpu_arrays[gaussian_frag_color_slang]->device_buffer, nanovdb_out,
                                1u);
    ...
    ctx->grid_build.grid_build_destroy(compute, queue, &grid_state);

    ctx->grid_build.fanout_state_destroy(compute, queue, &state);
                                            ...
                                            void* userdata)
{
    ...
    pnanovdb_raster_gaussian_data_t* data =
        create_gaussian_data(compute, queue, cast(ctx), means, quaternions, scales, colors, spherical_harmonics,
                             opacities, shader_params_arrays, shader_param_count);
                            ...
                            pnanovdb_compute_array_t* nanovdb_arr)
{
    ...
    printf("upper_count(%llu), lower_count(%llu), leaf_count(%llu)\n", (unsigned long long int)tree_upper_count,
           (unsigned long long int)tree_lower_count, (unsigned long long int)tree_leaf_count);
                ...
                if (pnanovdb_node2_get_child_mask_bit(buf, upper, PNANOVDB_NODE2_TYPE_UPPER, upper_n))
                {
                    ...
                    lower_sparse_child_bytes += pnanovdb_node2_compute_size(PNANOVDB_NODE2_TYPE_LOWER) -
                                                (pnanovdb_node2_type_fanout(PNANOVDB_NODE2_TYPE_LOWER) -
                                                 pnanovdb_node2_get_child_count(buf, lower, PNANOVDB_NODE2_TYPE_LOWER)) *
                                                    4u;
    ...
    for (pnanovdb_uint64_t point_idx = 0u; point_idx < point_count; point_idx++)
    {
        ...
        pnanovdb_node2_handle_t node = pnanovdb_node2_root_find_node(
            buf, root, root_flags, PNANOVDB_REF(ijk), PNANOVDB_REF(dense_n), PNANOVDB_REF(node_type));

        pnanovdb_address_t values = pnanovdb_grid_get_gridblindmetadata_value_address(buf, grid, 1u);
        pnanovdb_address_t val_addr =
            pnanovdb_node2_get_value_address(buf, root, root_flags, values, 32u, 0u, PNANOVDB_REF(ijk));
        if (point_idx < 64u)
        {
            pnanovdb_uint32_t value_raw = pnanovdb_read_uint32(buf, val_addr);
            float color[4] = { float((value_raw >> 0) & 255) * (1.f / 255.f),
                               float((value_raw >> 8) & 255) * (1.f / 255.f),
                               float((value_raw >> 16) & 255) * (1.f / 255.f),
                               float((value_raw >> 24) & 255) * (1.f / 255.f) };
            printf("val_addr(%llu) vcolor(%f,%f,%f,%f)\n", (unsigned long long int)val_addr.byte_offset, color[0],
