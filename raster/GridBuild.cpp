...
static const char* s_shader_names[shader_count] = {
    "raster/grid_build_blindmetadata.slang", "raster/grid_build_clear.slang",
    "raster/grid_build_compute_bbox.slang",  "raster/grid_build_count_new.slang",
    "raster/grid_build_finalize.slang",      "raster/grid_build_init.slang",
    "raster/grid_build_scatter_new.slang",   "raster/grid_build_set_masks.slang",

    "raster/fanout_clear_prim_idxs.slang",   "raster/fanout_compute_fragment_idxs.slang",
...
static pnanovdb_grid_build_context_t* create_context(const pnanovdb_compute_t* compute, pnanovdb_compute_queue_t* queue)
{
...
static int empty_grid_generate(void* dst_data, pnanovdb_uint64_t dst_size, float voxel_size)
{
    ...
    for (pnanovdb_uint32_t node_n = 0u; node_n < 2u * pnanovdb_node2_fanout_1d[PNANOVDB_NODE2_TYPE_ROOT]; node_n++)
    {
        pnanovdb_node2_write(
            buf, root.idx64 + pnanovdb_node2_off_children[PNANOVDB_NODE2_TYPE_ROOT] + node_n, pnanovdb_node2_end_key);
    }
...
static void grid_build_init(const pnanovdb_compute_t* compute,
                            pnanovdb_compute_queue_t* queue,
                            pnanovdb_grid_build_context_t* context_in,
                            ...
                            pnanovdb_uint32_t dispatch_count)
{
    ...
    state->node_types_buffer = compute_interface->create_buffer(context, PNANOVDB_COMPUTE_MEMORY_TYPE_DEVICE, &buf_desc);
    ...
    state->bboxes_buffer = compute_interface->create_buffer(context, PNANOVDB_COMPUTE_MEMORY_TYPE_DEVICE, &buf_desc);
}

static void grid_build_reset(const pnanovdb_compute_t* compute,
                             pnanovdb_compute_queue_t* queue,
                             pnanovdb_grid_build_context_t* context_in,
                             pnanovdb_grid_build_state_t* state,
                             pnanovdb_compute_buffer_t* nanovdb_out,
                             ...
                             pnanovdb_uint32_t dispatch_count)
{
    ...
    // clear nanovdb buf with empty grid
    {
        ...
        compute->dispatch_shader(compute_interface, context, ctx->shader_ctx[grid_build_clear_slang], resources,
                                 constants.workgroup_count, 1u, 1u, "grid_build_clear");
    }
    // initialize per node_idx buffers
    {
        ...
        compute->dispatch_shader(compute_interface, context, ctx->shader_ctx[grid_build_init_slang], resources,
                                 (constants.max_node_count + 255u) / 256u, 1u, 1u, "grid_build_init");
    }
...
static void grid_build_destroy(const pnanovdb_compute_t* compute,
                               pnanovdb_compute_queue_t* queue,
                               pnanovdb_grid_build_state_t* state)
{
    pnanovdb_compute_interface_t* compute_interface = compute->device_interface.get_compute_interface(queue);
...
static void grid_build(const pnanovdb_compute_t* compute,
                       pnanovdb_compute_queue_t* queue,
                       pnanovdb_grid_build_context_t* context_in,
                       pnanovdb_grid_build_state_t* state,
                       ...
                       pnanovdb_uint32_t dispatch_count)
{
    ...
    for (pnanovdb_uint32_t tree_level = 0u; tree_level < 4u; tree_level++)
    {
        ...
        // set masks
        {
            ...
            compute->dispatch_shader(compute_interface, context, ctx->shader_ctx[grid_build_set_masks_slang], resources,
                                     (constants.point_count + 255u) / 256u, 1u, 1u, "grid_build_set_masks");
        }
        // count new
        {
            ...
            compute->dispatch_shader(compute_interface, context, ctx->shader_ctx[grid_build_count_new_slang], resources,
                                     constants.workgroup_count, 1u, 1u, "grid_build_count_new");
        }
        // global scan new_child_counts
        {
            ctx->parallel_primitives.global_scan(compute, queue, ctx->parallel_primitives_ctx,
                                                 state->new_child_counts_buffer, state->scan_new_child_counts_buffer,
                                                 constants.max_node_count, 1u);
        }
        // scatter new
        {
            ...
            compute->dispatch_shader(compute_interface, context, ctx->shader_ctx[grid_build_scatter_new_slang],
                                     resources, constants.workgroup_count, 1u, 1u, "grid_build_scatter_new");
        }
        // finalize
        {
            pnanovdb_compute_resource_t resources[4u] = {};
            ...
            compute->dispatch_shader(compute_interface, context, ctx->shader_ctx[grid_build_finalize_slang], resources,
                                     1u, 1u, 1u, "grid_build_finalize");
        }
...
static void grid_build_finalize(const pnanovdb_compute_t* compute,
                                pnanovdb_compute_queue_t* queue,
                                pnanovdb_grid_build_context_t* context_in,
                                pnanovdb_grid_build_state_t* state,
                                pnanovdb_compute_buffer_t* point_ijk_in,
                                ...
                                pnanovdb_uint32_t dispatch_count)
{
    ...
    // add blindmetadata
    {
        ...
        compute->dispatch_shader(compute_interface, context, ctx->shader_ctx[grid_build_blindmetadata_slang], resources,
                                 1u, 1u, 1u, "grid_build_blindmetadata");
    ...
    for (pnanovdb_uint32_t tree_level = 3u; tree_level < 4u; tree_level--)
    {
        ...
        // compute bbox
        {
            ...
            compute->dispatch_shader(compute_interface, context, ctx->shader_ctx[grid_build_compute_bbox_slang],
                                     resources, constants.workgroup_count, 1u, 1u, "grid_build_compute_bbox");
        }
...
static void fanout_state_reset(const pnanovdb_compute_t* compute,
                               pnanovdb_compute_queue_t* queue,
                               pnanovdb_grid_build_context_t* context_in,
...
static pnanovdb_bool_t fanout_state_valid(const pnanovdb_compute_t* compute,
                                          pnanovdb_compute_queue_t* queue,
                                          pnanovdb_grid_build_context_t* context_in,
                                          pnanovdb_grid_build_fanout_state_t* state)
{
...
static void fanout_state_increment(const pnanovdb_compute_t* compute,
                                   pnanovdb_compute_queue_t* queue,
                                   pnanovdb_grid_build_context_t* context_in,
                                   pnanovdb_grid_build_fanout_state_t* state)
{
...
static void fanout_state_init(const pnanovdb_compute_t* compute,
                              pnanovdb_compute_queue_t* queue,
                              pnanovdb_grid_build_context_t* context_in,
                              pnanovdb_grid_build_fanout_state_t* state,
                              ...
                              float voxel_size)
{
    ...
    state->prim_bbox_buffer = compute_interface->create_buffer(context, PNANOVDB_COMPUTE_MEMORY_TYPE_DEVICE, &buf_desc);
    ...
    state->scatter_prim_idxs_buffer =
        compute_interface->create_buffer(context, PNANOVDB_COMPUTE_MEMORY_TYPE_DEVICE, &buf_desc);
    state->prim_idxs_buffer = compute_interface->create_buffer(context, PNANOVDB_COMPUTE_MEMORY_TYPE_DEVICE, &buf_desc);
    ...
    state->readback_buffer = compute_interface->create_buffer(context, PNANOVDB_COMPUTE_MEMORY_TYPE_READBACK, &buf_desc);
...
static void fanout_state_destroy(const pnanovdb_compute_t* compute,
                                 pnanovdb_compute_queue_t* queue,
                                 pnanovdb_grid_build_fanout_state_t* state)
{
    pnanovdb_compute_interface_t* compute_interface = compute->device_interface.get_compute_interface(queue);
...
static pnanovdb_compute_buffer_t* fanout_get_constant_buffer(const pnanovdb_compute_t* compute,
                                                             pnanovdb_compute_queue_t* queue,
                                                             pnanovdb_grid_build_fanout_state_t* state,
                                                             fanout_constants_t* constants_out)
{
    ...
    constants.global_prim_count = state->prim_count, constants.voxel_size = state->voxel_size;
...
static void fanout(const pnanovdb_compute_t* compute,
                   pnanovdb_compute_queue_t* queue,
                   pnanovdb_grid_build_context_t* context_in,
                   pnanovdb_grid_build_fanout_state_t* state,
                   pnanovdb_uint32_t dispatch_count)
{
    ...
    {
        ctx->parallel_primitives.global_scan_uint64(compute, queue, ctx->parallel_primitives_ctx,
                                                    state->fragment_counts_buffer, state->scan_fragment_counts_buffer,
                                                    constants.prim_batch_size, 1u);
    }
    ...
    else if (state->ijk_batch_count == state->ijk_batch_count_max)
    {
        ...
        {
            pnanovdb_uint64_t* mapped =
                (pnanovdb_uint64_t*)compute_interface->map_buffer(context, state->readback_buffer);
    ...
    // clear prim idxs
    {
        ...
        compute->dispatch_shader(compute_interface, context, ctx->shader_ctx[fanout_clear_prim_idxs_slang], resources,
                                 (constants.ijk_batch_size + 255u) / 256u, 1u, 1u, "fanout_clear_prim_idxs");
    }
    // scatter prim idxs
    {
        pnanovdb_compute_resource_t resources[4u] = {};
        ...
        compute->dispatch_shader(compute_interface, context, ctx->shader_ctx[fanout_scatter_prim_idxs_slang], resources,
                                 (constants.prim_batch_size + 255u) / 256u, 1u, 1u, "fanout_prim_idxs");
    }
    // global max scan to spread prim ids
    {
        ctx->parallel_primitives.global_scan_max(compute, queue, ctx->parallel_primitives_ctx,
                                                 state->scatter_prim_idxs_buffer, state->prim_idxs_buffer,
                                                 constants.ijk_batch_size, 1u);
    }
    // compute fragment idxs
    {
        ...
        compute->dispatch_shader(compute_interface, context, ctx->shader_ctx[fanout_compute_fragment_idxs_slang],
                                 resources, (constants.ijk_batch_size + 255u) / 256u, 1u, 1u,
