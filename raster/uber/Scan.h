...
PNANOVDB_FORCE_INLINE void pnanovdb_workgroup_scan(pnanovdb_uint32_t vidx_offset,
                                                   pnanovdb_buf_t smem_buf,
                                                   pnanovdb_address_t smem_addr,
                                                   pnanovdb_uvec4_t val[PNANOVDB_COMPUTE_VECTOR_WIDTH],
                                                   PNANOVDB_INOUT_ARRAY(pnanovdb_uvec4_t)
                                                       result[PNANOVDB_COMPUTE_VECTOR_WIDTH],
                                                   PNANOVDB_INOUT(pnanovdb_uint32_t) total_count)
{
    ...
    for (vidx = 0u; vidx < PNANOVDB_COMPUTE_VECTOR_WIDTH; vidx++)
    {
        ...
        local_val[vidx] = val[vidx].x + val[vidx].y + val[vidx].z + val[vidx].w;
    ...
    for (vidx = 0u; vidx < PNANOVDB_COMPUTE_VECTOR_WIDTH; vidx++)
    {
        ...
        if (thread_idx >= 1)
            local_val[vidx] += pnanovdb_read_uint32_index(smem_buf, sdata0_addr, thread_idx - 1);
        if (thread_idx >= 2)
            local_val[vidx] += pnanovdb_read_uint32_index(smem_buf, sdata0_addr, thread_idx - 2);
        if (thread_idx >= 3)
            local_val[vidx] += pnanovdb_read_uint32_index(smem_buf, sdata0_addr, thread_idx - 3);
    ...
    for (vidx = 0u; vidx < PNANOVDB_COMPUTE_VECTOR_WIDTH; vidx++)
    {
        ...
        if (thread_idx >= 4)
            local_val[vidx] += pnanovdb_read_uint32_index(smem_buf, sdata1_addr, thread_idx - 4);
        if (thread_idx >= 8)
            local_val[vidx] += pnanovdb_read_uint32_index(smem_buf, sdata1_addr, thread_idx - 8);
        if (thread_idx >= 12)
            local_val[vidx] += pnanovdb_read_uint32_index(smem_buf, sdata1_addr, thread_idx - 12);
        pnanovdb_write_uint32_index(smem_buf, sdata0_addr, thread_idx, local_val[vidx]);
    }
    ...
    for (vidx = 0u; vidx < PNANOVDB_COMPUTE_VECTOR_WIDTH; vidx++)
    {
        ...
        if (thread_idx >= 16)
            local_val[vidx] += pnanovdb_read_uint32_index(smem_buf, sdata0_addr, thread_idx - 16);
        if (thread_idx >= 32)
            local_val[vidx] += pnanovdb_read_uint32_index(smem_buf, sdata0_addr, thread_idx - 32);
        if (thread_idx >= 48)
            local_val[vidx] += pnanovdb_read_uint32_index(smem_buf, sdata0_addr, thread_idx - 48);
        pnanovdb_write_uint32_index(smem_buf, sdata1_addr, thread_idx, local_val[vidx]);
    }
    ...
    for (vidx = 0u; vidx < PNANOVDB_COMPUTE_VECTOR_WIDTH; vidx++)
    {
        ...
        if (thread_idx >= 64)
            local_val[vidx] += pnanovdb_read_uint32_index(smem_buf, sdata1_addr, thread_idx - 64);
        if (thread_idx >= 128)
            local_val[vidx] += pnanovdb_read_uint32_index(smem_buf, sdata1_addr, thread_idx - 128);
        if (thread_idx >= 192)
            local_val[vidx] += pnanovdb_read_uint32_index(smem_buf, sdata1_addr, thread_idx - 192);
...
PNANOVDB_FORCE_INLINE void pnanovdb_workgroup_scan_max(pnanovdb_uint32_t vidx_offset,
                                                       pnanovdb_buf_t smem_buf,
                                                       pnanovdb_address_t smem_addr,
                                                       pnanovdb_uvec4_t val[PNANOVDB_COMPUTE_VECTOR_WIDTH],
                                                       PNANOVDB_INOUT_ARRAY(pnanovdb_uvec4_t)
                                                       ...
                                                       PNANOVDB_INOUT(pnanovdb_uint32_t) total_count)
{
    ...
    for (vidx = 0u; vidx < PNANOVDB_COMPUTE_VECTOR_WIDTH; vidx++)
    {
        ...
        local_val[vidx] = pnanovdb_uint32_max(
            pnanovdb_uint32_max(val[vidx].x, val[vidx].y), pnanovdb_uint32_max(val[vidx].z, val[vidx].w));
        pnanovdb_write_uint32_index(smem_buf, sdata0_addr, thread_idx, local_val[vidx]);
    }
    ...
    for (vidx = 0u; vidx < PNANOVDB_COMPUTE_VECTOR_WIDTH; vidx++)
    {
        ...
        if (thread_idx >= 1)
            local_val[vidx] =
                pnanovdb_uint32_max(local_val[vidx], pnanovdb_read_uint32_index(smem_buf, sdata0_addr, thread_idx - 1));
        if (thread_idx >= 2)
            local_val[vidx] =
                pnanovdb_uint32_max(local_val[vidx], pnanovdb_read_uint32_index(smem_buf, sdata0_addr, thread_idx - 2));
        if (thread_idx >= 3)
            local_val[vidx] =
                pnanovdb_uint32_max(local_val[vidx], pnanovdb_read_uint32_index(smem_buf, sdata0_addr, thread_idx - 3));
        pnanovdb_write_uint32_index(smem_buf, sdata1_addr, thread_idx, local_val[vidx]);
    }
    ...
    for (vidx = 0u; vidx < PNANOVDB_COMPUTE_VECTOR_WIDTH; vidx++)
    {
        ...
        if (thread_idx >= 4)
            local_val[vidx] =
                pnanovdb_uint32_max(local_val[vidx], pnanovdb_read_uint32_index(smem_buf, sdata1_addr, thread_idx - 4));
        if (thread_idx >= 8)
            local_val[vidx] =
                pnanovdb_uint32_max(local_val[vidx], pnanovdb_read_uint32_index(smem_buf, sdata1_addr, thread_idx - 8));
        if (thread_idx >= 12)
            local_val[vidx] =
                pnanovdb_uint32_max(local_val[vidx], pnanovdb_read_uint32_index(smem_buf, sdata1_addr, thread_idx - 12));
        pnanovdb_write_uint32_index(smem_buf, sdata0_addr, thread_idx, local_val[vidx]);
    }
    ...
    for (vidx = 0u; vidx < PNANOVDB_COMPUTE_VECTOR_WIDTH; vidx++)
    {
        ...
        if (thread_idx >= 16)
            local_val[vidx] =
                pnanovdb_uint32_max(local_val[vidx], pnanovdb_read_uint32_index(smem_buf, sdata0_addr, thread_idx - 16));
        if (thread_idx >= 32)
            local_val[vidx] =
                pnanovdb_uint32_max(local_val[vidx], pnanovdb_read_uint32_index(smem_buf, sdata0_addr, thread_idx - 32));
        if (thread_idx >= 48)
            local_val[vidx] =
                pnanovdb_uint32_max(local_val[vidx], pnanovdb_read_uint32_index(smem_buf, sdata0_addr, thread_idx - 48));
        pnanovdb_write_uint32_index(smem_buf, sdata1_addr, thread_idx, local_val[vidx]);
    }
    ...
    for (vidx = 0u; vidx < PNANOVDB_COMPUTE_VECTOR_WIDTH; vidx++)
    {
        ...
        if (thread_idx >= 64)
            local_val[vidx] =
                pnanovdb_uint32_max(local_val[vidx], pnanovdb_read_uint32_index(smem_buf, sdata1_addr, thread_idx - 64));
        if (thread_idx >= 128)
            local_val[vidx] = pnanovdb_uint32_max(
                local_val[vidx], pnanovdb_read_uint32_index(smem_buf, sdata1_addr, thread_idx - 128));
        if (thread_idx >= 192)
...
PNANOVDB_FORCE_INLINE float pnanovdb_workgroup_scan_keyed_get_float_if_match(pnanovdb_uint32_t thread_idx,
                                                                             pnanovdb_buf_t smem_buf,
...
PNANOVDB_FORCE_INLINE void pnanovdb_workgroup_scan_keyed_float(pnanovdb_uint32_t vidx_offset,
                                                               pnanovdb_buf_t smem_buf,
                                                               pnanovdb_address_t smem_addr,
                                                               pnanovdb_vec4_t val[PNANOVDB_COMPUTE_VECTOR_WIDTH],
                                                               pnanovdb_uvec4_t key[PNANOVDB_COMPUTE_VECTOR_WIDTH],
                                                               PNANOVDB_INOUT_ARRAY(pnanovdb_vec4_t)
                                                                   result[PNANOVDB_COMPUTE_VECTOR_WIDTH],
                                                               ...
                                                               PNANOVDB_INOUT(pnanovdb_uint32_t) last_key)
{
    ...
    for (vidx = 0u; vidx < PNANOVDB_COMPUTE_VECTOR_WIDTH; vidx++)
    {
        ...
        if (key[vidx].w == key[vidx].z)
        {
            local_val[vidx] += val[vidx].z;
        }
        if (key[vidx].w == key[vidx].y)
        {
            local_val[vidx] += val[vidx].y;
        }
        if (key[vidx].w == key[vidx].x)
        {
    ...
    for (vidx = 0u; vidx < PNANOVDB_COMPUTE_VECTOR_WIDTH; vidx++)
    {
        ...
        local_val[vidx] += pnanovdb_workgroup_scan_keyed_get_float_if_match(
            thread_idx, smem_buf, sdata0_addr, skey_addr, key[vidx].w, 1u);
        local_val[vidx] += pnanovdb_workgroup_scan_keyed_get_float_if_match(
            thread_idx, smem_buf, sdata0_addr, skey_addr, key[vidx].w, 2u);
        local_val[vidx] += pnanovdb_workgroup_scan_keyed_get_float_if_match(
            thread_idx, smem_buf, sdata0_addr, skey_addr, key[vidx].w, 3u);
    ...
    for (vidx = 0u; vidx < PNANOVDB_COMPUTE_VECTOR_WIDTH; vidx++)
    {
        ...
        local_val[vidx] += pnanovdb_workgroup_scan_keyed_get_float_if_match(
            thread_idx, smem_buf, sdata1_addr, skey_addr, key[vidx].w, 4u);
        local_val[vidx] += pnanovdb_workgroup_scan_keyed_get_float_if_match(
            thread_idx, smem_buf, sdata1_addr, skey_addr, key[vidx].w, 8u);
        local_val[vidx] += pnanovdb_workgroup_scan_keyed_get_float_if_match(
            thread_idx, smem_buf, sdata1_addr, skey_addr, key[vidx].w, 12u);
        pnanovdb_write_float_index(smem_buf, sdata0_addr, thread_idx, local_val[vidx]);
    }
    ...
    for (vidx = 0u; vidx < PNANOVDB_COMPUTE_VECTOR_WIDTH; vidx++)
    {
        ...
        local_val[vidx] += pnanovdb_workgroup_scan_keyed_get_float_if_match(
            thread_idx, smem_buf, sdata0_addr, skey_addr, key[vidx].w, 16u);
        local_val[vidx] += pnanovdb_workgroup_scan_keyed_get_float_if_match(
            thread_idx, smem_buf, sdata0_addr, skey_addr, key[vidx].w, 32u);
        local_val[vidx] += pnanovdb_workgroup_scan_keyed_get_float_if_match(
            thread_idx, smem_buf, sdata0_addr, skey_addr, key[vidx].w, 48u);
        pnanovdb_write_float_index(smem_buf, sdata1_addr, thread_idx, local_val[vidx]);
    }
    ...
    for (vidx = 0u; vidx < PNANOVDB_COMPUTE_VECTOR_WIDTH; vidx++)
    {
        ...
        local_val[vidx] += pnanovdb_workgroup_scan_keyed_get_float_if_match(
            thread_idx, smem_buf, sdata1_addr, skey_addr, key[vidx].w, 64u);
        local_val[vidx] += pnanovdb_workgroup_scan_keyed_get_float_if_match(
            thread_idx, smem_buf, sdata1_addr, skey_addr, key[vidx].w, 48u);
        local_val[vidx] += pnanovdb_workgroup_scan_keyed_get_float_if_match(
            thread_idx, smem_buf, sdata1_addr, skey_addr, key[vidx].w, 192u);
        pnanovdb_write_float_index(smem_buf, sdata0_addr, thread_idx, local_val[vidx]);
    }
...
PNANOVDB_FORCE_INLINE void pnanovdb_scan(pnanovdb_uint32_t vidx_offset,
                                         pnanovdb_buf_t smem_buf,
                                         pnanovdb_address_t smem_addr,
                                         ...
                                         pnanovdb_buf_t val_buf,
                                         pnanovdb_address_t val_addr,
                                         pnanovdb_buf_t tmp_buf,
                                         pnanovdb_address_t tmp_addr,
                                         pnanovdb_buf_t result_buf,
                                         pnanovdb_address_t result_addr,
                                         PNANOVDB_INOUT(pnanovdb_uint32_t) total_count)
{
    pnanovdb_uint32_t workgroup_count = (val_count + 1023u) / 1024u;
    ...
    while (pnanovdb_sync_get_workgroup_idx(sync_state, smem_buf, smem_sync_addr, sync_buf, sync_addr))
    {
        ...
        for (vidx = 0u; vidx < PNANOVDB_COMPUTE_VECTOR_WIDTH; vidx++)
        {
            ...
            if (4u * val4_idx + 0u >= val_count)
            {
                val[vidx].x = 0u;
            }
            if (4u * val4_idx + 1u >= val_count)
            {
                val[vidx].y = 0u;
            }
            if (4u * val4_idx + 2u >= val_count)
            {
                val[vidx].z = 0u;
            }
            if (4u * val4_idx + 3u >= val_count)
            {
                val[vidx].w = 0u;
            }
        }
    ...
    PNANOVDB_DEREF(total_count) =
        workgroup_count > 0u ? pnanovdb_read_uint32_index(tmp_buf, reduce_scan_addr, workgroup_count - 1u) : 0u;
}
...
PNANOVDB_FORCE_INLINE void pnanovdb_scan_max(pnanovdb_uint32_t vidx_offset,
                                             pnanovdb_buf_t smem_buf,
                                             pnanovdb_address_t smem_addr,
                                             pnanovdb_address_t smem_sync_addr,
                                             PNANOVDB_INOUT(pnanovdb_sync_state_t) sync_state,
...
PNANOVDB_FORCE_INLINE pnanovdb_address_t pnanovdb_scan_keyed_float_compute_tmp_size(pnanovdb_uint32_t val_count,
                                                                                    pnanovdb_uint32_t channel_count)
{
    ...
    pnanovdb_address_t reduce_val_addr =
        pnanovdb_alloc_address_aligned32(PNANOVDB_REF(tmp_addr), channel_count * workgroup_count * 4u);
    pnanovdb_address_t reduce_scan_addr =
        pnanovdb_alloc_address_aligned32(PNANOVDB_REF(tmp_addr), channel_count * workgroup_count * 4u);
    return tmp_addr;
}

PNANOVDB_FORCE_INLINE void pnanovdb_scan_keyed_float(pnanovdb_uint32_t vidx_offset,
                                                     pnanovdb_buf_t smem_buf,
                                                     pnanovdb_address_t smem_addr,
                                                     pnanovdb_address_t smem_sync_addr,
                                                     PNANOVDB_INOUT(pnanovdb_sync_state_t) sync_state,
                                                     ...
                                                     pnanovdb_address_t result_addr)
{
    ...
    while (pnanovdb_sync_get_workgroup_idx(sync_state, smem_buf, smem_sync_addr, sync_buf, sync_addr))
    {
        ...
        for (pnanovdb_uint32_t channel_idx = 0u; channel_idx < channel_count; channel_idx++)
        {
            ...
            for (vidx = 0u; vidx < PNANOVDB_COMPUTE_VECTOR_WIDTH; vidx++)
            {
                ...
                {
                    val[vidx].x =
                        pnanovdb_read_float_index(val_buf, val_addr, channel_count * (4u * val4_idx + 0u) + channel_idx);
                    val[vidx].y =
                        pnanovdb_read_float_index(val_buf, val_addr, channel_count * (4u * val4_idx + 1u) + channel_idx);
                    val[vidx].z =
                        pnanovdb_read_float_index(val_buf, val_addr, channel_count * (4u * val4_idx + 2u) + channel_idx);
                    val[vidx].w =
                        pnanovdb_read_float_index(val_buf, val_addr, channel_count * (4u * val4_idx + 3u) + channel_idx);
                }
                if (4u * val4_idx + 0u >= val_count || key.x != key_max)
                {
                    val[vidx].x = 0u;
                }
                if (4u * val4_idx + 1u >= val_count || key.y != key_max)
                {
                    val[vidx].y = 0u;
                }
                if (4u * val4_idx + 2u >= val_count || key.z != key_max)
                {
                    val[vidx].z = 0u;
                }
                if (4u * val4_idx + 3u >= val_count || key.w != key_max)
                {
                    val[vidx].w = 0u;
                }
            }
            ...
            if (vidx_offset == 0u)
            {
                ...
                pnanovdb_write_float_index(
                    tmp_buf, reduce_val_addr, channel_count * reduce_idx + channel_idx, total_count);
        ...
        for (pnanovdb_uint32_t channel_idx = 0u; channel_idx < channel_count; channel_idx++)
        {
            ...
            for (pnanovdb_uint32_t scan_pass_idx = 0u; scan_pass_idx < scan_pass_count; scan_pass_idx++)
            {
                ...
                for (vidx = 0u; vidx < PNANOVDB_COMPUTE_VECTOR_WIDTH; vidx++)
                {
                    ...
                    if (4u * reduce_idx < workgroup_count)
                    {
                        reduce[vidx].x = pnanovdb_read_float_index(
                            tmp_buf, reduce_val_addr, channel_count * (4u * reduce_idx + 0u) + channel_idx);
                        reduce[vidx].y = pnanovdb_read_float_index(
                            tmp_buf, reduce_val_addr, channel_count * (4u * reduce_idx + 1u) + channel_idx);
                        reduce[vidx].z = pnanovdb_read_float_index(
                            tmp_buf, reduce_val_addr, channel_count * (4u * reduce_idx + 2u) + channel_idx);
                        reduce[vidx].w = pnanovdb_read_float_index(
                            tmp_buf, reduce_val_addr, channel_count * (4u * reduce_idx + 3u) + channel_idx);
                ...
                pnanovdb_workgroup_scan_keyed_float(vidx_offset, smem_buf, smem_addr, reduce, key, reduce_scan,
                                                    PNANOVDB_REF(last_count), PNANOVDB_REF(last_key));
                ...
                for (vidx = 0u; vidx < PNANOVDB_COMPUTE_VECTOR_WIDTH; vidx++)
                {
                    ...
                    if (key[vidx].x == global_key)
                    {
                        reduce_scan[vidx].x += global_offset;
                    }
                    if (key[vidx].y == global_key)
                    {
                        reduce_scan[vidx].y += global_offset;
                    };
                    if (key[vidx].z == global_key)
                    {
                        reduce_scan[vidx].z += global_offset;
                    };
                    if (key[vidx].w == global_key)
                    {
                        reduce_scan[vidx].w += global_offset;
                    };
                    ...
                    if (4u * reduce_idx < workgroup_count)
                    {
                        pnanovdb_write_float_index(tmp_buf, reduce_scan_addr,
                                                   channel_count * (4u * reduce_idx + 0u) + channel_idx,
                                                   reduce_scan[vidx].x);
                        pnanovdb_write_float_index(tmp_buf, reduce_scan_addr,
                                                   channel_count * (4u * reduce_idx + 1u) + channel_idx,
                                                   reduce_scan[vidx].y);
                        pnanovdb_write_float_index(tmp_buf, reduce_scan_addr,
                                                   channel_count * (4u * reduce_idx + 2u) + channel_idx,
                                                   reduce_scan[vidx].z);
                        pnanovdb_write_float_index(tmp_buf, reduce_scan_addr,
                                                   channel_count * (4u * reduce_idx + 3u) + channel_idx,
                                                   reduce_scan[vidx].w);
                    }
                }
    ...
    while (pnanovdb_sync_get_workgroup_idx(sync_state, smem_buf, smem_sync_addr, sync_buf, sync_addr))
    {
        for (pnanovdb_uint32_t channel_idx = 0u; channel_idx < channel_count; channel_idx++)
        {
            ...
            pnanovdb_workgroup_scan_keyed_float(
                vidx_offset, smem_buf, smem_addr, val, key, val_scan, PNANOVDB_REF(last_count), PNANOVDB_REF(last_key));
            ...
            if (workgroup_idx > 0u)
            {
                global_offset = pnanovdb_read_float_index(
                    tmp_buf, reduce_scan_addr, channel_count * (workgroup_idx - 1u) + channel_idx);
            ...
            for (vidx = 0u; vidx < PNANOVDB_COMPUTE_VECTOR_WIDTH; vidx++)
            {
                ...
                {
                    pnanovdb_write_float_index(
                        result_buf, result_addr, channel_count * (4u * val4_idx + 0u) + channel_idx, val_scan[vidx].x);
                    pnanovdb_write_float_index(
                        result_buf, result_addr, channel_count * (4u * val4_idx + 1u) + channel_idx, val_scan[vidx].y);
                    pnanovdb_write_float_index(
                        result_buf, result_addr, channel_count * (4u * val4_idx + 2u) + channel_idx, val_scan[vidx].z);
                    pnanovdb_write_float_index(
                        result_buf, result_addr, channel_count * (4u * val4_idx + 3u) + channel_idx, val_scan[vidx].w);
                }
            }
        }
    }
