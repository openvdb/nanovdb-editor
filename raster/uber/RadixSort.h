...
{
    if (bucket == 0)
        PNANOVDB_DEREF(counter).x += (1 << 0);
    if (bucket == 1)
        PNANOVDB_DEREF(counter).x += (1 << 16);
    if (bucket == 2)
        PNANOVDB_DEREF(counter).y += (1 << 0);
    if (bucket == 3)
        PNANOVDB_DEREF(counter).y += (1 << 16);
    if (bucket == 4)
        PNANOVDB_DEREF(counter).z += (1 << 0);
    if (bucket == 5)
        PNANOVDB_DEREF(counter).z += (1 << 16);
    if (bucket == 6)
        PNANOVDB_DEREF(counter).w += (1 << 0);
    if (bucket == 7)
        PNANOVDB_DEREF(counter).w += (1 << 16);
    if (bucket == 8)
        PNANOVDB_DEREF(counter).x += (1 << 8);
    if (bucket == 9)
        PNANOVDB_DEREF(counter).x += (1 << 24);
    if (bucket == 10)
        PNANOVDB_DEREF(counter).y += (1 << 8);
    if (bucket == 11)
        PNANOVDB_DEREF(counter).y += (1 << 24);
    if (bucket == 12)
        PNANOVDB_DEREF(counter).z += (1 << 8);
    if (bucket == 13)
        PNANOVDB_DEREF(counter).z += (1 << 24);
    if (bucket == 14)
        PNANOVDB_DEREF(counter).w += (1 << 8);
    if (bucket == 15)
        PNANOVDB_DEREF(counter).w += (1 << 24);
}
...
PNANOVDB_FORCE_INLINE void pnanovdb_radix_sort1(pnanovdb_uint32_t vidx_offset,
                                                ...
                                                pnanovdb_buf_t key_in_buf,
                                                pnanovdb_address_t key_in_addr,
                                                pnanovdb_buf_t counters_out_buf,
                                                pnanovdb_address_t counters_out_addr)
{
    ...
    for (vidx = 0u; vidx < PNANOVDB_COMPUTE_VECTOR_WIDTH; vidx++)
    {
        ...
        if (4u * key4_idx + 0u >= key_count)
        {
            key_local.x = 0xFFFFFFFF;
        }
        if (4u * key4_idx + 1u >= key_count)
        {
            key_local.y = 0xFFFFFFFF;
        }
        if (4u * key4_idx + 2u >= key_count)
        {
            key_local.z = 0xFFFFFFFF;
        }
        if (4u * key4_idx + 3u >= key_count)
        {
            key_local.w = 0xFFFFFFFF;
        }
    ...
    for (vidx = 0u; vidx < PNANOVDB_COMPUTE_VECTOR_WIDTH; vidx++)
    {
        ...
        if (thread_idx < 256u / 4u)
        {
            ...
            local_count = pnanovdb_uvec4_add(
                local_count, pnanovdb_read_uvec4_index(smem_buf, scount0_addr, thread_idx + 1 * 256u / 4u));
            local_count = pnanovdb_uvec4_add(
                local_count, pnanovdb_read_uvec4_index(smem_buf, scount0_addr, thread_idx + 2 * 256u / 4u));
            local_count = pnanovdb_uvec4_add(
                local_count, pnanovdb_read_uvec4_index(smem_buf, scount0_addr, thread_idx + 3 * 256u / 4u));
    ...
    for (vidx = 0u; vidx < PNANOVDB_COMPUTE_VECTOR_WIDTH; vidx++)
    {
        ...
        if (thread_idx < 256u / 16u)
        {
            ...
            local_counth = pnanovdb_uvec4_add(local_counth, pnanovdb_radix_expand8to16h(local_raw));
            pnanovdb_write_uvec4(smem_buf, pnanovdb_address_offset(scount0_addr, thread_idx * 16u), local_count);
            pnanovdb_write_uvec4(
                smem_buf, pnanovdb_address_offset(scount0_addr, (thread_idx + 256u / 16u) * 16u), local_counth);
        }
    }
    pnanovdb_workgroup_sync();
    ...
    for (vidx = 0u; vidx < PNANOVDB_COMPUTE_VECTOR_WIDTH; vidx++)
    {
        ...
        if (set_lane_id < 256u / 64u)
        {
            ...
            pnanovdb_write_uvec4(
                smem_buf, pnanovdb_address_offset(scount1_addr, (set_lane_id + set_id * 256u / 64) * 16u), local_count);
        }
    }
    pnanovdb_workgroup_sync();
    ...
    for (vidx = 0u; vidx < PNANOVDB_COMPUTE_VECTOR_WIDTH; vidx++)
    {
        ...
        if (set_lane_id == 0)
        {
            ...
            pnanovdb_write_uint32_index(counters_out_buf, counters_out_addr,
                                        (bucket_offset + 0) * workgroup_count + workgroup_idx,
                                        (local_count.x & 0x0000FFFF) >> 0);
            pnanovdb_write_uint32_index(counters_out_buf, counters_out_addr,
                                        (bucket_offset + 1) * workgroup_count + workgroup_idx,
                                        (local_count.x & 0xFFFF0000) >> 16);
            pnanovdb_write_uint32_index(counters_out_buf, counters_out_addr,
                                        (bucket_offset + 2) * workgroup_count + workgroup_idx,
                                        (local_count.y & 0x0000FFFF) >> 0);
            pnanovdb_write_uint32_index(counters_out_buf, counters_out_addr,
                                        (bucket_offset + 3) * workgroup_count + workgroup_idx,
                                        (local_count.y & 0xFFFF0000) >> 16);
            pnanovdb_write_uint32_index(counters_out_buf, counters_out_addr,
                                        (bucket_offset + 4) * workgroup_count + workgroup_idx,
                                        (local_count.z & 0x0000FFFF) >> 0);
            pnanovdb_write_uint32_index(counters_out_buf, counters_out_addr,
                                        (bucket_offset + 5) * workgroup_count + workgroup_idx,
                                        (local_count.z & 0xFFFF0000) >> 16);
            pnanovdb_write_uint32_index(counters_out_buf, counters_out_addr,
                                        (bucket_offset + 6) * workgroup_count + workgroup_idx,
                                        (local_count.w & 0x0000FFFF) >> 0);
            pnanovdb_write_uint32_index(counters_out_buf, counters_out_addr,
                                        (bucket_offset + 7) * workgroup_count + workgroup_idx,
                                        (local_count.w & 0xFFFF0000) >> 16);

            // output counter values for local scan
            pnanovdb_write_uint32_index(counters_out_buf, counters_out_addr,
                                        (bucket_offset + 0) + 16 * (workgroup_count + workgroup_idx),
                                        (local_count.x & 0x0000FFFF) >> 0);
            pnanovdb_write_uint32_index(counters_out_buf, counters_out_addr,
                                        (bucket_offset + 1) + 16 * (workgroup_count + workgroup_idx),
                                        (local_count.x & 0xFFFF0000) >> 16);
            pnanovdb_write_uint32_index(counters_out_buf, counters_out_addr,
                                        (bucket_offset + 2) + 16 * (workgroup_count + workgroup_idx),
                                        (local_count.y & 0x0000FFFF) >> 0);
            pnanovdb_write_uint32_index(counters_out_buf, counters_out_addr,
                                        (bucket_offset + 3) + 16 * (workgroup_count + workgroup_idx),
                                        (local_count.y & 0xFFFF0000) >> 16);
            pnanovdb_write_uint32_index(counters_out_buf, counters_out_addr,
                                        (bucket_offset + 4) + 16 * (workgroup_count + workgroup_idx),
                                        (local_count.z & 0x0000FFFF) >> 0);
            pnanovdb_write_uint32_index(counters_out_buf, counters_out_addr,
                                        (bucket_offset + 5) + 16 * (workgroup_count + workgroup_idx),
                                        (local_count.z & 0xFFFF0000) >> 16);
            pnanovdb_write_uint32_index(counters_out_buf, counters_out_addr,
                                        (bucket_offset + 6) + 16 * (workgroup_count + workgroup_idx),
                                        (local_count.w & 0x0000FFFF) >> 0);
            pnanovdb_write_uint32_index(counters_out_buf, counters_out_addr,
                                        (bucket_offset + 7) + 16 * (workgroup_count + workgroup_idx),
                                        (local_count.w & 0xFFFF0000) >> 16);
        }
    }
}
...
PNANOVDB_FORCE_INLINE void pnanovdb_radix_sort2(pnanovdb_uint32_t vidx_offset,
                                                pnanovdb_uint32_t workgroup_idx,
                                                pnanovdb_buf_t smem_buf,
                                                pnanovdb_address_t smem_addr,
                                                pnanovdb_uint32_t key_count,
                                                pnanovdb_buf_t counters_in_buf,
                                                pnanovdb_address_t counters_in_addr,
                                                pnanovdb_buf_t counters_out_buf,
                                                pnanovdb_address_t counters_out_addr)
{
    ...
    if (workgroup_idx == 0u)
    {
        ...
        for (pnanovdb_uint32_t passID = 0; passID < numPasses; passID++)
        {
            ...
            pnanovdb_workgroup_scan(
                vidx_offset, smem_buf, smem_addr, count_local, count_global, PNANOVDB_REF(workgroup_offset));
    ...
    else if (workgroup_idx == 1u)
    {
        ...
        for (pnanovdb_uint32_t passID = 0; passID < numPasses; passID++)
        {
            ...
            for (vidx = 0u; vidx < PNANOVDB_COMPUTE_VECTOR_WIDTH; vidx++)
            {
                ...
                if (idx[vidx] < (16u * workgroup_count) / 4)
                {
                    count_local[vidx] = pnanovdb_read_uvec4_index(
                        counters_in_buf, counters_in_addr, idx[vidx] + (16u * workgroup_count) / 4);
                }

                pnanovdb_uint32_t count_local_32 =
                    count_local[vidx].x + count_local[vidx].y + count_local[vidx].z + count_local[vidx].w;
            ...
            for (vidx = 0u; vidx < PNANOVDB_COMPUTE_VECTOR_WIDTH; vidx++)
            {
                ...
                if (idx[vidx] < (16u * workgroup_count) / 4)
                {
                    pnanovdb_write_uvec4_index(counters_out_buf, counters_out_addr,
                                               idx[vidx] + (16u * workgroup_count) / 4, count_local[vidx]);
                }
...
PNANOVDB_FORCE_INLINE void pnanovdb_radix_split4(pnanovdb_uint32_t vidx_offset,
                                                 pnanovdb_buf_t smem_buf,
                                                 pnanovdb_address_t smem_addr,
                                                 pnanovdb_uvec4_t pred[PNANOVDB_COMPUTE_VECTOR_WIDTH],
                                                 PNANOVDB_INOUT_ARRAY(pnanovdb_uvec4_t)
                                                     rank[PNANOVDB_COMPUTE_VECTOR_WIDTH])
{
...
PNANOVDB_FORCE_INLINE void pnanovdb_radix_sort3(pnanovdb_uint32_t vidx_offset,
                                                pnanovdb_uint32_t workgroup_idx,
                                                pnanovdb_buf_t smem_buf,
                                                pnanovdb_address_t smem_addr,
                                                pnanovdb_uint32_t pass_start,
                                                ...
                                                pnanovdb_buf_t key_in_buf,
                                                pnanovdb_address_t key_in_addr,
                                                pnanovdb_buf_t val_in_buf,
                                                pnanovdb_address_t val_in_addr,
                                                pnanovdb_buf_t counters_in_buf,
                                                pnanovdb_address_t counters_in_addr,
                                                pnanovdb_buf_t key_out_buf,
                                                pnanovdb_address_t key_out_addr,
                                                pnanovdb_buf_t val_out_buf,
                                                pnanovdb_address_t val_out_addr)
{
    ...
    for (vidx = 0u; vidx < PNANOVDB_COMPUTE_VECTOR_WIDTH; vidx++)
    {
        ...
        if (4u * key4_idx + 0u >= key_count)
        {
            key_local[vidx].x = 0xFFFFFFFF;
            val_local[vidx].x = 0u;
        }
        if (4u * key4_idx + 1u >= key_count)
        {
            key_local[vidx].y = 0xFFFFFFFF;
            val_local[vidx].y = 0u;
        }
        if (4u * key4_idx + 2u >= key_count)
        {
            key_local[vidx].z = 0xFFFFFFFF;
            val_local[vidx].z = 0u;
        }
        if (4u * key4_idx + 3u >= key_count)
        {
            key_local[vidx].w = 0xFFFFFFFF;
            val_local[vidx].w = 0u;
        }
    }
    ...
    for (vidx = 0u; vidx < PNANOVDB_COMPUTE_VECTOR_WIDTH; vidx++)
    {
        ...
        for (pnanovdb_uint32_t shared_idx = thread_idx; shared_idx < 4 * 256u; shared_idx += 256u)
        {
            ...
            pnanovdb_uint32_t dst_idx = shared_idx -
                                        pnanovdb_read_uint32_index(counters_in_buf, counters_in_addr,
                                                                   bucket_idx + 16 * (workgroup_count + workgroup_idx)) +
                                        pnanovdb_read_uint32_index(counters_in_buf, counters_in_addr,
                                                                   bucket_idx * workgroup_count + workgroup_idx);
...
PNANOVDB_FORCE_INLINE void pnanovdb_radix_sort(pnanovdb_uint32_t vidx_offset,
                                               pnanovdb_buf_t smem_buf,
                                               pnanovdb_address_t smem_addr,
                                               ...
                                               pnanovdb_address_t tmp_addr)
{
    ...
    for (pnanovdb_uint32_t pass_id = 0; pass_id < num_key_bits; pass_id += 4u)
    {
        ...
        {
            pnanovdb_radix_sort1(PNANOVDB_DEREF(sync_state).vidx_offset, PNANOVDB_DEREF(sync_state).workgroup_idx,
                                 smem_buf, smem_addr, pass_start, pass_num_bits, pass_mask, key_count, key_buf,
                                 key_addr, tmp_buf, counters_a_addr);
        }

        pnanovdb_sync(sync_state, 2u, smem_buf, smem_sync_addr, sync_buf, sync_addr);
        while (pnanovdb_sync_get_workgroup_idx(sync_state, smem_buf, smem_sync_addr, sync_buf, sync_addr))
        {
            pnanovdb_radix_sort2(PNANOVDB_DEREF(sync_state).vidx_offset, PNANOVDB_DEREF(sync_state).workgroup_idx,
                                 smem_buf, smem_addr, key_count, tmp_buf, counters_a_addr, tmp_buf, counters_b_addr);
        }

        pnanovdb_sync(sync_state, workgroup_count, smem_buf, smem_sync_addr, sync_buf, sync_addr);
        while (pnanovdb_sync_get_workgroup_idx(sync_state, smem_buf, smem_sync_addr, sync_buf, sync_addr))
        {
            pnanovdb_radix_sort3(PNANOVDB_DEREF(sync_state).vidx_offset, PNANOVDB_DEREF(sync_state).workgroup_idx,
                                 smem_buf, smem_addr, pass_start, pass_num_bits, pass_mask, key_count, key_buf,
                                 key_addr, val_buf, val_addr, tmp_buf, counters_b_addr, tmp_buf, key_tmp_addr, tmp_buf,
                                 val_tmp_addr);
        }
        pnanovdb_sync(sync_state, workgroup_count, smem_buf, smem_sync_addr, sync_buf, sync_addr);
        ...
        {
            pnanovdb_radix_sort1(PNANOVDB_DEREF(sync_state).vidx_offset, PNANOVDB_DEREF(sync_state).workgroup_idx,
                                 smem_buf, smem_addr, pass_start, pass_num_bits, pass_mask, key_count, tmp_buf,
                                 key_tmp_addr, tmp_buf, counters_a_addr);
        }
        ...
        while (pnanovdb_sync_get_workgroup_idx(sync_state, smem_buf, smem_sync_addr, sync_buf, sync_addr))
        {
            pnanovdb_radix_sort3(PNANOVDB_DEREF(sync_state).vidx_offset, PNANOVDB_DEREF(sync_state).workgroup_idx,
                                 smem_buf, smem_addr, pass_start, pass_num_bits, pass_mask, key_count, tmp_buf,
                                 key_tmp_addr, tmp_buf, val_tmp_addr, tmp_buf, counters_b_addr, key_buf, key_addr,
                                 val_buf, val_addr);
        }
        pnanovdb_sync(sync_state, workgroup_count, smem_buf, smem_sync_addr, sync_buf, sync_addr);
    }
...
PNANOVDB_FORCE_INLINE pnanovdb_address_t pnanovdb_radix_sort_96_compute_tmp_size(pnanovdb_uint32_t key_count,
                                                                                 pnanovdb_uint32_t element_size)
{
    pnanovdb_address_t tmp_addr = pnanovdb_address_null();
    pnanovdb_address_t radix_sort_addr =
        pnanovdb_alloc_address_aligned(PNANOVDB_REF(tmp_addr), pnanovdb_radix_sort_compute_tmp_size(key_count));
    pnanovdb_address_t key_tmp_addr = pnanovdb_alloc_address_aligned32(PNANOVDB_REF(tmp_addr), key_count * 4u);
    pnanovdb_address_t val_tmp_addr = pnanovdb_alloc_address_aligned32(PNANOVDB_REF(tmp_addr), key_count * 4u);
...
PNANOVDB_FORCE_INLINE void pnanovdb_radix_sort_96(pnanovdb_uint32_t vidx_offset,
                                                  pnanovdb_buf_t smem_buf,
                                                  pnanovdb_address_t smem_addr,
                                                  pnanovdb_address_t smem_sync_addr,
                                                  PNANOVDB_INOUT(pnanovdb_sync_state_t) sync_state,
                                                  ...
                                                  pnanovdb_address_t tmp_addr)
{
    ...
    pnanovdb_radix_sort(vidx_offset, smem_buf, smem_addr, smem_sync_addr, sync_state, sync_buf, sync_addr, 32u,
                        key_count, tmp_buf, key_tmp_addr, tmp_buf, val_tmp_addr, tmp_buf, radix_sort_addr);
    ...
    while (pnanovdb_sync_get_workgroup_idx(sync_state, smem_buf, smem_sync_addr, sync_buf, sync_addr))
    {
        ...
        for (vidx = 0u; vidx < PNANOVDB_COMPUTE_VECTOR_WIDTH; vidx++)
        {
            ...
            if (dst_idx < key_count)
            {
                ...
                {
                    pnanovdb_uint32_t val =
                        pnanovdb_read_uint32_index(val_buf, val_addr, (element_size / 4u) * src_idx + (sub_idx / 4u));

                    pnanovdb_write_uint32_index(
                        tmp_buf, copy_val_addr, (element_size / 4u) * dst_idx + (sub_idx / 4u), val);
                }
            }
        }
    }
    ...
    while (pnanovdb_sync_get_workgroup_idx(sync_state, smem_buf, smem_sync_addr, sync_buf, sync_addr))
    {
        ...
        for (vidx = 0u; vidx < PNANOVDB_COMPUTE_VECTOR_WIDTH; vidx++)
        {
            ...
            if (dst_idx < key_count)
            {
                ...
                for (sub_idx = 0u; sub_idx < element_size; sub_idx += 4u)
                {
                    pnanovdb_uint32_t val = pnanovdb_read_uint32_index(
                        tmp_buf, copy_val_addr, (element_size / 4u) * dst_idx + (sub_idx / 4u));
