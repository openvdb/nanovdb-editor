...
pnanovdb_imgui_texture_t* create_texture(pnanovdb_compute_context_t* context,
                                         pnanovdb_imgui_renderer_t* renderer,
                                         unsigned char* pixels,
                                         int texWidth,
                                         int texHeight);

pnanovdb_imgui_renderer_t* create(const pnanovdb_compute_t* compute,
                                  ...
                                  int texHeight)
{
    ...
    pnanovdb_compute_buffer_usage_t bufferUsage =
        PNANOVDB_COMPUTE_BUFFER_USAGE_STRUCTURED | PNANOVDB_COMPUTE_BUFFER_USAGE_COPY_SRC;

    pnanovdb_compute_upload_buffer_init(&ptr->compute_interface, context, &ptr->vertexPosTexCoordBuffer, bufferUsage,
                                        PNANOVDB_COMPUTE_FORMAT_UNKNOWN, 4u * sizeof(float));
    pnanovdb_compute_upload_buffer_init(&ptr->compute_interface, context, &ptr->vertexColorBuffer, bufferUsage,
                                        PNANOVDB_COMPUTE_FORMAT_UNKNOWN, sizeof(pnanovdb_uint32_t));
    pnanovdb_compute_upload_buffer_init(&ptr->compute_interface, context, &ptr->indicesBuffer, bufferUsage,
                                        PNANOVDB_COMPUTE_FORMAT_UNKNOWN, sizeof(pnanovdb_uint32_t));
    pnanovdb_compute_upload_buffer_init(&ptr->compute_interface, context, &ptr->drawCmdsBuffer, bufferUsage,
                                        PNANOVDB_COMPUTE_FORMAT_UNKNOWN, sizeof(imgui_renderer_draw_cmd_t));
    pnanovdb_compute_upload_buffer_init(&ptr->compute_interface, context, &ptr->constantBuffer,
                                        PNANOVDB_COMPUTE_BUFFER_USAGE_CONSTANT, PNANOVDB_COMPUTE_FORMAT_UNKNOWN, 0u);

    pnanovdb_compute_upload_buffer_init(
        &ptr->compute_interface, context, &ptr->textureUpload, bufferUsage, PNANOVDB_COMPUTE_FORMAT_UNKNOWN, 4u);
    pnanovdb_compute_upload_buffer_init(
        &ptr->compute_interface, context, &ptr->textureTableUpload, bufferUsage, PNANOVDB_COMPUTE_FORMAT_UNKNOWN, 4u);
    ...
    pnanovdb_compute_buffer_usage_t deviceBufUsage =
        PNANOVDB_COMPUTE_BUFFER_USAGE_RW_STRUCTURED | PNANOVDB_COMPUTE_BUFFER_USAGE_STRUCTURED;

    pnanovdb_compute_dynamic_buffer_init(&ptr->compute_interface, context, &ptr->treeBuffer, deviceBufUsage,
                                         PNANOVDB_COMPUTE_FORMAT_UNKNOWN, 4u * sizeof(int));
    pnanovdb_compute_dynamic_buffer_init(&ptr->compute_interface, context, &ptr->tileCountBuffer, deviceBufUsage,
                                         PNANOVDB_COMPUTE_FORMAT_UNKNOWN, sizeof(pnanovdb_uint32_t));
    pnanovdb_compute_dynamic_buffer_init(&ptr->compute_interface, context, &ptr->triangleBuffer, deviceBufUsage,
                                         PNANOVDB_COMPUTE_FORMAT_UNKNOWN, sizeof(pnanovdb_uint32_t));
    pnanovdb_compute_dynamic_buffer_init(&ptr->compute_interface, context, &ptr->triangleRangeBuffer, deviceBufUsage,
                                         PNANOVDB_COMPUTE_FORMAT_UNKNOWN, 2u * sizeof(pnanovdb_uint32_t));
    ...
    ptr->totalCountBuffer =
        ptr->compute_interface.create_buffer(context, PNANOVDB_COMPUTE_MEMORY_TYPE_READBACK, &totalCountDesc);
            ...
            pnanovdb_compute_texture_transient_t* colorOut)
{
    ...
    if (ptr->textureDirty)
    {
        ...
        for (pnanovdb_uint64_t textureIdx = 0u; textureIdx < ptr->textures.size(); textureIdx++)
        {
            ...
            tileGridOffset += ptr->textures[textureIdx]->tileGridWidth * ptr->textures[textureIdx]->tileGridHeight;
        }
        ...
        *((float*)&ptr->textureTable[3u]) = 1.f / float(atlasHeight); // atlasHeightInv
        ...
        auto mappedTable = (pnanovdb_uint32_t*)pnanovdb_compute_upload_buffer_map(
            context, &ptr->textureTableUpload, ptr->textureTable.size() * 4u);
        for (pnanovdb_uint64_t idx = 0u; idx < ptr->textureTable.size(); idx++)
        {
            mappedTable[idx] = ptr->textureTable[idx];
        }
        pnanovdb_compute_upload_buffer_unmap_device(
            context, &ptr->textureTableUpload, 0llu, ptr->textureTable.size() * 4u, "ImguiTextureTableUpload");
        ...
        for (pnanovdb_uint64_t textureIdx = 0u; textureIdx < ptr->textures.size(); textureIdx++)
        {
            ...
            {
                memcpy(mapped + uploadOffset * 18u * 18u * 4u, ptr->textures[textureIdx]->tiles[tileIdx].data,
                       18u * 18u * 4u);
                uploadOffset++;
            }
        }
        pnanovdb_compute_buffer_transient_t* uploadTransient =
            pnanovdb_compute_upload_buffer_unmap(context, &ptr->textureUpload);
        ...
        pnanovdb_compute_texture_transient_t* textureTransient =
            ptr->compute_interface.register_texture_as_transient(context, ptr->textureDevice);

        {
            ...
            compute->dispatch_shader(&ptr->compute_interface, context, ptr->shader_context[imgui_texture_upload_cs],
                                     resources, tileGridOffset, 1u, 1u, "imgui_texture_upload");
        }
    }
    ...
    auto mappedPosTex =
        (float*)pnanovdb_compute_upload_buffer_map(context, &ptr->vertexPosTexCoordBuffer, numBytesPosTex);
    auto mappedColor =
        (pnanovdb_uint32_t*)pnanovdb_compute_upload_buffer_map(context, &ptr->vertexColorBuffer, numBytesColor);
    auto mappedIndices =
        (pnanovdb_uint32_t*)pnanovdb_compute_upload_buffer_map(context, &ptr->indicesBuffer, numBytesIndices);
    auto mappedDrawCmds =
        (imgui_renderer_draw_cmd_t*)pnanovdb_compute_upload_buffer_map(context, &ptr->drawCmdsBuffer, numBytesDrawCmds);
    auto mapped = (imgui_renderer_params_t*)pnanovdb_compute_upload_buffer_map(
        context, &ptr->constantBuffer, sizeof(imgui_renderer_params_t));
    ...
    for (int cmdListIdx = 0u; cmdListIdx < drawData->CmdListsCount; cmdListIdx++)
    {
        ...
        for (int drawCmdIdx = 0; drawCmdIdx < cmdList->CmdBuffer.Size; drawCmdIdx++)
        {
            ...
            dst.userTexture =
                src.TexRef.GetTexID() ? cast((pnanovdb_imgui_texture_t*)src.TexRef.GetTexID())->textureId : 0u;
    ...
    // pnanovdb_compute_buffer_transient_t* vertexPosTexCoordTransient =
    // pnanovdb_compute_upload_buffer_unmap_device(context, &ptr->vertexPosTexCoordBuffer, 0llu, numBytesPosTex);
    // pnanovdb_compute_buffer_transient_t* vertexColorTransient = pnanovdb_compute_upload_buffer_unmap_device(context,
    // &ptr->vertexColorBuffer, 0llu, numBytesColor); pnanovdb_compute_buffer_transient_t* indicesTransient =
    // pnanovdb_compute_upload_buffer_unmap_device(context, &ptr->indicesBuffer, 0llu, numBytesIndices);
    // pnanovdb_compute_buffer_transient_t* drawCmdsInTransient = pnanovdb_compute_upload_buffer_unmap_device(context,
    // &ptr->drawCmdsBuffer, 0llu, numBytesDrawCmds); pnanovdb_compute_buffer_transient_t* paramsInTransient =
    // pnanovdb_compute_upload_buffer_unmap(context, &ptr->constantBuffer);

    pnanovdb_compute_buffer_transient_t* vertexPosTexCoordTransient =
        pnanovdb_compute_upload_buffer_unmap(context, &ptr->vertexPosTexCoordBuffer);
    pnanovdb_compute_buffer_transient_t* vertexColorTransient =
        pnanovdb_compute_upload_buffer_unmap(context, &ptr->vertexColorBuffer);
    pnanovdb_compute_buffer_transient_t* indicesTransient =
        pnanovdb_compute_upload_buffer_unmap(context, &ptr->indicesBuffer);
    pnanovdb_compute_buffer_transient_t* drawCmdsInTransient =
        pnanovdb_compute_upload_buffer_unmap(context, &ptr->drawCmdsBuffer);
    pnanovdb_compute_buffer_transient_t* paramsInTransient =
        pnanovdb_compute_upload_buffer_unmap(context, &ptr->constantBuffer);

    pnanovdb_compute_texture_transient_t* textureTransient =
        ptr->compute_interface.register_texture_as_transient(context, ptr->textureDevice);
    pnanovdb_compute_buffer_transient_t* textureTableTransient =
        ptr->compute_interface.register_buffer_as_transient(context, ptr->textureTableUpload.device_buffer);
    pnanovdb_compute_buffer_transient_t* treeTransient =
        pnanovdb_compute_dynamic_buffer_get_transient(context, &ptr->treeBuffer);
    pnanovdb_compute_buffer_transient_t* tileCountTransient =
        pnanovdb_compute_dynamic_buffer_get_transient(context, &ptr->tileCountBuffer);
    pnanovdb_compute_buffer_transient_t* triangleTransient =
        pnanovdb_compute_dynamic_buffer_get_transient(context, &ptr->triangleBuffer);
    pnanovdb_compute_buffer_transient_t* triangleRangeTransient =
        pnanovdb_compute_dynamic_buffer_get_transient(context, &ptr->triangleRangeBuffer);
    ...
    pnanovdb_compute_buffer_transient_t* totalCountTransient =
        ptr->compute_interface.register_buffer_as_transient(context, ptr->totalCountBuffer);
    ...
    // build acceleration structure
    {
        ...
        compute->dispatch_shader(&ptr->compute_interface, context, ptr->shader_context[imgui_build_cs], resources,
                                 numBlocks, 1u, 1u, "imgui_build");
    }
    ...
    // count triangles per tile
    {
        ...
        compute->dispatch_shader(&ptr->compute_interface, context, ptr->shader_context[imgui_tile_count_cs], resources,
                                 (tileGridDim_xy + 255u) / 256u, 1u, 1u, "imgui_tile_count");
    }
    ...
    // scan buckets
    {
        ...
        compute->dispatch_shader(&ptr->compute_interface, context, ptr->shader_context[imgui_tile_scan_cs], resources,
                                 1u, 1u, 1u, "imgui_tile_scan");
    }
    ...
    // generate tile data
    {
        ...
        compute->dispatch_shader(&ptr->compute_interface, context, ptr->shader_context[imgui_tile_cs], resources,
                                 (tileGridDim_xy + 255u) / 256u, 1u, 1u, "imgui_tile");
    }
    ...
    if (!colorIn)
    {
        ...
        compute->dispatch_shader(&ptr->compute_interface, context, ptr->shader_context[imgui_background_cs], resources,
                                 (width + 7u) / 8u, (height + 7u) / 8u, 1u, "imgui_background");
    }
    ...
    // render
    {
        ...
        compute->dispatch_shader(&ptr->compute_interface, context, ptr->shader_context[imgui_cs], resources,
                                 (width + 7u) / 8u, (height + 7u) / 8u, 1u, "imgui_render");
    }
}

void copy_texture(const pnanovdb_compute_t* compute,
                  pnanovdb_compute_context_t* context,
                  pnanovdb_imgui_renderer_t* renderer,
                  pnanovdb_uint32_t width,
                  pnanovdb_uint32_t height,
                  ...
                  pnanovdb_compute_texture_transient_t* colorOut)
{
    ...
    compute->dispatch_shader(&ptr->compute_interface, context, ptr->shader_context[imgui_copy_texture_cs], resources,
                             (width + 7u) / 8u, (height + 7u) / 8u, 1u, "imgui_copy_texture");
}

void copy_texture_yuv(const pnanovdb_compute_t* compute,
                      pnanovdb_compute_context_t* context,
                      pnanovdb_imgui_renderer_t* renderer,
                      pnanovdb_uint32_t width,
                      pnanovdb_uint32_t height,
                      pnanovdb_compute_texture_transient_t* colorIn,
                      pnanovdb_compute_texture_transient_t* plane0Out,
                      pnanovdb_compute_texture_transient_t* plane1Out,
                      pnanovdb_compute_texture_transient_t* plane2Out)
{
    auto ptr = cast(renderer);

    pnanovdb_compute_resource_t resources[4u] = {};
    resources[0u].texture_transient = colorIn;
    resources[1u].texture_transient = plane0Out;
    resources[2u].texture_transient = plane1Out;
    resources[3u].texture_transient = plane2Out;

    pnanovdb_uint32_t shader_id = plane2Out ? imgui_copy_texture_yuv3_cs : imgui_copy_texture_yuv2_cs;

    compute->dispatch_shader(&ptr->compute_interface, context, ptr->shader_context[shader_id], resources,
                             (width + 7u) / 8u, (height + 7u) / 8u, 1u,
...
void destroy_texture(pnanovdb_compute_context_t* context,
                     pnanovdb_imgui_renderer_t* renderer,
                     pnanovdb_imgui_texture_t* texture)
{
    auto ptr = cast(renderer);
...
pnanovdb_imgui_renderer_interface_t* pnanovdb_imgui_get_renderer_interface()
{
    ...
    static pnanovdb_imgui_renderer_interface_t iface = { PNANOVDB_REFLECT_INTERFACE_INIT(
        pnanovdb_imgui_renderer_interface_t) };
    iface.create = create;
    iface.destroy = destroy;
    iface.render = render;
    iface.copy_texture = copy_texture;
    iface.copy_texture_yuv = copy_texture_yuv;
