...
PNANOVDB_INLINE void pnanovdb_compute_dynamic_buffer_init(pnanovdb_compute_interface_t* compute_interface,
...
PNANOVDB_INLINE void pnanovdb_compute_dynamic_buffer_destroy(pnanovdb_compute_context_t* context,
                                                             pnanovdb_compute_dynamic_buffer_t* ptr)
{
...
PNANOVDB_INLINE void pnanovdb_compute_dynamic_buffer_resize(pnanovdb_compute_context_t* context,
                                                            pnanovdb_compute_dynamic_buffer_t* ptr,
                                                            pnanovdb_uint64_t num_bytes)
{
    ...
    if (!ptr->device_buffer)
    {
        ...
        ptr->device_buffer =
            ptr->compute_interface->create_buffer(context, PNANOVDB_COMPUTE_MEMORY_TYPE_DEVICE, &bufDesc);
        ptr->transient_buffer = nullptr;
        ptr->transient_frame = ~0llu;
    }
}
...
PNANOVDB_INLINE pnanovdb_compute_buffer_transient_t* pnanovdb_compute_dynamic_buffer_get_transient(
    pnanovdb_compute_context_t* context, pnanovdb_compute_dynamic_buffer_t* ptr)
{
    pnanovdb_compute_frame_info_t frame_info = {};
    ptr->compute_interface->get_frame_info(context, &frame_info);

    if (ptr->transient_buffer && ptr->transient_frame == frame_info.frame_local_current)
    {
